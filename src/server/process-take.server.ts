...
export async function runProcessTake(
  ...
  try {
    ...
    try {
      ...
      const briefSourceText = typeof audition.brief === 'string' && audition.brief.trim().length > 0
        ? audition.brief
        : typeof audition.extracted_brief === 'string' && audition.extracted_brief.trim().length > 0
