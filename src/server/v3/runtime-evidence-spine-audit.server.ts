...
const RUNTIME_EVIDENCE_SPINE_AUDIT_MAP: ReadonlyArray<RuntimeEvidenceSpineAuditItem> = [
  ...
  { artefact_id: 'resolver_output', category: 'analysis_take', expected_path: 'resolver/resolver_output.json', current_source_module: 'required list only / TruthStateMap type nearby if present', expected_source_of_truth: 'deterministic post-processing/resolver stage', current_manifest_status: 'missing', source_classification: 'missing', runtime_data_available_now: 'unknown', can_emit_without_invention: false, required_for_level: 'L2', blocker_code: 'resolver_output_missing', next_implementation_step: 'Identify concrete resolver structure then emit' },
  { artefact_id: 'truth_state_map', category: 'analysis_take', expected_path: 'resolver/TruthStateMap.json', current_source_module: 'src/server/v3/types.ts type + required list', expected_source_of_truth: 'resolver truth-state computation', current_manifest_status: 'missing', source_classification: 'source_only_stub', runtime_data_available_now: 'unknown', can_emit_without_invention: false, required_for_level: 'L2', blocker_code: 'TruthStateMap_missing', next_implementation_step: 'Implement true runtime truth map emitter only after source identified' },
  ...
  { artefact_id: 'technique_observation_trace', category: 'analysis_take', expected_path: 'traces/TechniqueObservationTrace.json', current_source_module: 'required list only', expected_source_of_truth: 'technique observation pipeline', current_manifest_status: 'missing', source_classification: 'missing', runtime_data_available_now: 'unknown', can_emit_without_invention: false, required_for_level: 'L2', blocker_code: 'TechniqueObservation_trace_missing', next_implementation_step: 'Emit from technique observations once canonical mapping is defined' },
  { artefact_id: 'score_trace', category: 'analysis_take', expected_path: 'traces/ScoreTrace.json', current_source_module: 'required list only', expected_source_of_truth: 'deterministic scoring pipeline', current_manifest_status: 'missing', source_classification: 'missing', runtime_data_available_now: 'unknown', can_emit_without_invention: false, required_for_level: 'L2', blocker_code: 'ScoreTrace_missing', next_implementation_step: 'Emit full score derivation trace' },
  { artefact_id: 'validator_trace', category: 'analysis_take', expected_path: 'traces/validator_trace.json', current_source_module: 'required list only', expected_source_of_truth: 'validation / UK / leakage gate chain', current_manifest_status: 'missing', source_classification: 'missing', runtime_data_available_now: 'unknown', can_emit_without_invention: false, required_for_level: 'L2', blocker_code: 'validator_trace_missing', next_implementation_step: 'Emit validator decisions trace' },
  { artefact_id: 'gate_trace', category: 'analysis_take', expected_path: 'traces/gate_trace.json', current_source_module: 'required list only', expected_source_of_truth: 'gate status computation', current_manifest_status: 'missing', source_classification: 'missing', runtime_data_available_now: 'unknown', can_emit_without_invention: false, required_for_level: 'L2', blocker_code: 'gate_trace_missing', next_implementation_step: 'Emit gate decision trace with blocker reasons' },
  { artefact_id: 'model_run_trace', category: 'analysis_take', expected_path: 'traces/ModelRunTrace.json', current_source_module: 'src/server/v3/qa-artifacts-wiring.server.ts (emitModelRunTraceArtefact)', expected_source_of_truth: 'model routing + attempts/fallback metadata', current_manifest_status: 'missing', source_classification: 'emitted_not_wired', runtime_data_available_now: true, can_emit_without_invention: true, required_for_level: 'L2', blocker_code: 'ModelRunTrace_missing', next_implementation_step: 'Wire existing emitter call in process pipeline only in later S9 task' },
  { artefact_id: 'qa_acceptance_metrics', category: 'analysis_take', expected_path: 'qa/acceptance_metrics.json', current_source_module: 'manifest has inline object only', expected_source_of_truth: 'qa summary file emitter', current_manifest_status: 'missing', source_classification: 'missing', runtime_data_available_now: true, can_emit_without_invention: false, required_for_level: 'L2', next_implementation_step: 'Emit dedicated qa/acceptance_metrics.json in later S9 task' },
