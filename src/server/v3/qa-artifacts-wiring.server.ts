...
export async function emitValidatorTraceFirstPass(input: any) {
  ...
  assertSafeSegment(input.take_id, 'take_id');
  assertSafeSegment(input.analysis_run_id ?? input.run_id, 'analysis_run_id');
  const entries = [
