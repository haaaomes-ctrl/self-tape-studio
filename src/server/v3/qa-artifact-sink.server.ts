...
export interface QAArtifactWriteResult { written: boolean; sink_mode: QAArtifactSinkMode; sink_write_status: 'ok'|'failed'; warning?: string; storage_bucket?: string; storage_path?: string; path?: string; log_fallback_emitted: boolean; }
...
function emitLog(input: { sink_mode: QAArtifactSinkMode; sink_write_status: 'ok'|'failed'; run_id: string; fixture_id?: string; artefact_id?: string; relative_path: string; storage_bucket?: string; storage_path?: string; blocker_codes?: string[]; payload: unknown; warning?: string; }) {
  const line = {
    ...
    payload_summary: { warning: input.warning ?? null, payload_bytes: Buffer.byteLength(stableStringify(input.payload), 'utf8') },
    internal_only: true,
  };
...
export async function writeQAArtifact(input: QAArtifactWriteInput): Promise<QAArtifactWriteResult> {
  ...
  try {
    if (mode === 'file') {
      ...
      if (allowLogFallback) emitLog({ sink_mode: mode, sink_write_status: 'ok', run_id: input.run_id, fixture_id: input.fixture_id, artefact_id: input.artefact_id, relative_path: input.relative_path, payload: input.payload });
      return { written: true, sink_mode: mode, sink_write_status: 'ok', path: abs, log_fallback_emitted: allowLogFallback };
    }
    if (mode === 'storage') {
      ...
      if (allowLogFallback) emitLog({ sink_mode: mode, sink_write_status: 'ok', run_id: input.run_id, fixture_id: input.fixture_id, artefact_id: input.artefact_id, relative_path: input.relative_path, storage_bucket, storage_path, payload: input.payload });
      return { written: true, sink_mode: mode, sink_write_status: 'ok', storage_bucket, storage_path, log_fallback_emitted: allowLogFallback };
    }
    emitLog({ sink_mode: mode, sink_write_status: 'ok', run_id: input.run_id, fixture_id: input.fixture_id, artefact_id: input.artefact_id, relative_path: input.relative_path, payload: input.payload });
    return { written: true, sink_mode: mode, sink_write_status: 'ok', log_fallback_emitted: true };
  } catch (error) {
    ...
    if (allowLogFallback) {
      ...
      return { written: true, sink_mode: mode, sink_write_status: 'failed', warning, storage_bucket: mode === 'storage' ? storage_bucket : undefined, storage_path: mode === 'storage' ? storage_path : undefined, log_fallback_emitted: true };
    }
    return { written: false, sink_mode: mode, sink_write_status: 'failed', warning, log_fallback_emitted: false };
  }
}
