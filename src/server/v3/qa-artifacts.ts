...
const REQUIRED: Omit<QARequiredArtefact, 'status' | 'blocker_code' | 'reason'>[] = [
  ...
  { artefact_id: 'validator_trace', name: 'validator trace', expected_path: 'traces/validator_trace.json', category: 'analysis_run', required_for_level: 'L2', linked_artifacts: [] },
  { artefact_id: 'redaction_trace', name: 'redaction trace', expected_path: 'traces/redaction_trace.json', category: 'analysis_run', required_for_level: 'L2', linked_artifacts: [] },
  { artefact_id: 'uk_gate', name: 'UK English gate', expected_path: 'traces/UKEnglishGateResult.json', category: 'analysis_run', required_for_level: 'L2', linked_artifacts: [] },
  { artefact_id: 'public_private_leakage', name: 'public private leakage', expected_path: 'traces/public_private_leakage_result.json', category: 'analysis_run', required_for_level: 'L2', linked_artifacts: [] },
  { artefact_id: 'gate_trace', name: 'gate trace', expected_path: 'traces/gate_trace.json', category: 'analysis_run', required_for_level: 'L2', linked_artifacts: [] },
  ...
  { artefact_id: 'parity_report', name: 'report parity', expected_path: 'parity/raw_to_render_parity.json', category: 'analysis_run', required_for_level: 'L2', linked_artifacts: [] },
  ...
  { artefact_id: 'parity_redaction', name: 'redaction parity', expected_path: 'parity/redaction_parity.json', category: 'analysis_run', required_for_level: 'L2', linked_artifacts: [] },
  { artefact_id: 'parity_timestamp', name: 'timestamp parity', expected_path: 'parity/timestamp_parity.json', category: 'analysis_run', required_for_level: 'L2', linked_artifacts: [] },
  { artefact_id: 'parity_technique_display', name: 'technique display parity', expected_path: 'parity/technique_display_status_parity.json', category: 'analysis_run', required_for_level: 'L2', linked_artifacts: [] },
...
const BLOCKERS: Record<string, string> = {
  ...
  comparison_suppression_trace: 'comparison_suppression_trace_missing', no_export_proof: 'no_export_proof_missing', parity_report: 'parity_artefacts_missing', parity_comparison: 'parity_artefacts_missing', parity_redaction:'parity_artefacts_missing', parity_timestamp:'parity_artefacts_missing', parity_technique_display:'parity_artefacts_missing', validator_trace:'validator_trace_missing', redaction_trace:'redaction_trace_missing', uk_gate:'UKEnglishGateResult_missing', public_private_leakage:'public_private_leakage_result_missing', gate_trace:'validator_trace_missing',
};
...
export async function emitInternalQAArtifactManifest(options: QAArtifactEmitterOptions) {
  ...
  const manifest = {
    ...
    level2_bundle_status: emitted_artifacts.length===0 ? 'not_started' : 'partial',
    level2_qa_acceptance: 'not_accepted',
  };
  await mkdir(runDir, { recursive: true });
  await writeFile(path.join(runDir, 'manifest.json'), stableStringify(manifest) + '\n', 'utf8');
  return { written: true, manifest_path: path.join(runDir, 'manifest.json'), manifest };
}
