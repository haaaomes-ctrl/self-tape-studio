...
export async function emitInternalQAArtifactManifest(options: QAArtifactEmitterOptions) {
  ...
  const rootReadmeExists = existsSync(path.join(process.cwd(), 'README.md'));
  const sourceScopeFile = options.source_scope_file ?? (rootReadmeExists ? 'README.md' : 'docs/tapecoach/v3/PROJECT_SCOPE_AND_QA_APPROACH.md');
  ...
  const manifest = {
    ...
    qa_artifact_root: runDir, source_scope_file: sourceScopeFile, controlling_source_file: sourceScopeFile, controlling_source_location_note: usingRootReadme ? 'Using repository root README.md as controlling requirements source' : 'Replacement README supplied externally; root README.md not present in workspace', controlling_requirements_status: usingRootReadme ? 'root_readme_present' : 'operator_supplied_replacement_README', fixture_refs: options.fixture_refs ?? [], input_refs: options.input_refs ?? [], take_refs: options.take_refs ?? [], mux_playback_ids: options.mux_playback_ids ?? {}, public_output_unchanged: true, user_experience_unchanged: true,
