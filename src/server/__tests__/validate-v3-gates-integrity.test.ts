...
describe('validate-v3-gates script integrity', () => {
  ...
  it('does not count validator text in shell comments', () => {
    const failures = runWithScripts({
      'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts',
      'gate:release': 'node scripts/validate-v3-gates.mjs # node scripts/validate-protected-areas.mjs\nnode scripts/validate-storage-bundle.mjs',
    });
    expect(failures.join('\n')).toContain('missing protected-area validation coverage');
  });

  it('does not count printf validator-looking text as executable coverage', () => {
    const failures = runWithScripts({
      'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts',
      'gate:release': 'node scripts/validate-protected-areas.mjs && printf \"node scripts/validate-storage-bundle.mjs\" && node scripts/validate-v3-gates.mjs',
    });
    expect(failures.join('\n')).toContain('missing Storage bundle validation coverage');
  });

  it('does not count node -e output as test:contracts coverage', () => {
    const failures = runWithScripts({
      'test:contracts': "node -e \"console.log('vitest run src/server/__tests__/v3-contracts.test.ts')\"",
      'gate:release': 'node scripts/validate-protected-areas.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-v3-gates.mjs',
    });
    expect(failures.join('\n')).toContain('test:contracts missing contract test execution coverage');
  });
  ...
  it('fails when gatekeeper workflow is missing pull_request trigger', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
  ...
  it('fails when contracts/build workflows are missing pull_request trigger', () => {
    ...
    const output = failures.join('\n');
    expect(output).toContain('contracts workflow missing pull_request trigger');
    expect(output).toContain('build workflow missing pull_request trigger');
  });

  it('fails when gatekeeper sets protected-area exceptions from workflow env', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: 'on:\n  pull_request:\njobs:\n  gatekeeper:\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      - run: npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: 49\n          PROTECTED_AREA_EXCEPTIONS_FILE: ./repo-file.json', contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
    });
  ...
  it('fails when gatekeeper sets inline protected-area exceptions JSON', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: 'on:\n  pull_request:\njobs:\n  gatekeeper:\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      - run: npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: ${{ github.event.pull_request.number }}\n          PROTECTED_AREA_EXCEPTIONS_JSON: {\"schema_version\":\"tapecoach_protected_area_exceptions_v1\"}', contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
    });
    expect(failures.join('\n')).toContain('must not set protected-area exception env vars');
  });

  it('fails when gatekeeper sets top-level protected-area exceptions JSON env', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: 'on:\n  pull_request:\nenv:\n  PROTECTED_AREA_EXCEPTIONS_JSON: ${{ vars.PROTECTED_AREA_EXCEPTIONS_JSON }}\njobs:\n  gatekeeper:\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      - run: npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: ${{ github.event.pull_request.number }}', contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
    });
    expect(failures.join('\n')).toContain('must not set protected-area exception env vars');
  });

  it('fails when gatekeeper sets job-level protected-area exceptions JSON env', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: 'on:\n  pull_request:\njobs:\n  gatekeeper:\n    env:\n      PROTECTED_AREA_EXCEPTIONS_JSON: ${{ vars.PROTECTED_AREA_EXCEPTIONS_JSON }}\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      - run: npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: ${{ github.event.pull_request.number }}', contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
  ...
  it('fails when non-gate step sets protected-area exceptions JSON env', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: 'on:\n  pull_request:\njobs:\n  gatekeeper:\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      - run: echo ok\n        env:\n          PROTECTED_AREA_EXCEPTIONS_JSON: ${{ vars.PROTECTED_AREA_EXCEPTIONS_JSON }}\n      - run: npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: ${{ github.event.pull_request.number }}', contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
  ...
  it('fails when gatekeeper writes protected-area exceptions env to GITHUB_ENV', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: 'on:\n  pull_request:\njobs:\n  gatekeeper:\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      - run: echo \"PROTECTED_AREA_EXCEPTIONS_JSON={}\" >> $GITHUB_ENV\n      - run: npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: ${{ github.event.pull_request.number }}', contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
    });
    expect(failures.join('\n')).toContain('must not set protected-area exception env vars');
  });

  it('fails when gatekeeper writes unquoted protected-area exceptions env to GITHUB_ENV', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: 'on:\n  pull_request:\njobs:\n  gatekeeper:\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      - run: echo PROTECTED_AREA_EXCEPTIONS_JSON={} >> $GITHUB_ENV\n      - run: npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: ${{ github.event.pull_request.number }}', contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
  ...
  it('fails when gatekeeper writes single-quoted protected-area exceptions env to GITHUB_ENV', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: "on:\n  pull_request:\njobs:\n  gatekeeper:\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      - run: echo 'PROTECTED_AREA_EXCEPTIONS_JSON={}' >> $GITHUB_ENV\n      - run: npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: ${{ github.event.pull_request.number }}", contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
  ...
  it('fails when gatekeeper writes protected-area exceptions env to quoted GITHUB_ENV target', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: 'on:\n  pull_request:\njobs:\n  gatekeeper:\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      - run: echo PROTECTED_AREA_EXCEPTIONS_JSON={} >> \"$GITHUB_ENV\"\n      - run: npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: ${{ github.event.pull_request.number }}', contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
  ...
  it('fails when gatekeeper writes protected-area exceptions env to braced GITHUB_ENV target', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: 'on:\n  pull_request:\njobs:\n  gatekeeper:\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      - run: echo PROTECTED_AREA_EXCEPTIONS_JSON={} >> ${GITHUB_ENV}\n      - run: npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: ${{ github.event.pull_request.number }}', contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
  ...
  it('fails when gatekeeper uses printf for protected-area exceptions GITHUB_ENV injection', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: 'on:\n  pull_request:\njobs:\n  gatekeeper:\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      - run: printf \"PROTECTED_AREA_EXCEPTIONS_JSON={}\" >> $GITHUB_ENV\n      - run: npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: ${{ github.event.pull_request.number }}', contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
    });
    expect(failures.join('\n')).toContain('must not set protected-area exception env vars');
  });

  it('fails when gatekeeper exports protected-area exceptions env in run command', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: 'on:\n  pull_request:\njobs:\n  gatekeeper:\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      - run: export PROTECTED_AREA_EXCEPTIONS_JSON=\"{}\" && npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: ${{ github.event.pull_request.number }}', contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
    });
    expect(failures.join('\n')).toContain('must not set protected-area exception env vars');
  });

  it('fails when gatekeeper sets protected-area exceptions env inline before gate command', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: 'on:\n  pull_request:\njobs:\n  gatekeeper:\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      - run: PROTECTED_AREA_EXCEPTIONS_JSON=\"{}\" npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: ${{ github.event.pull_request.number }}', contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
    });
    expect(failures.join('\n')).toContain('must not set protected-area exception env vars');
  });
  ...
  it('fails when gatekeeper uses deprecated set-env protected-area exception injection', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: 'on:\n  pull_request:\njobs:\n  gatekeeper:\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      - run: echo \"::set-env name=PROTECTED_AREA_EXCEPTIONS_JSON::{}\"\n      - run: npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: ${{ github.event.pull_request.number }}', contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
    });
    expect(failures.join('\n')).toContain('must not set protected-area exception env vars');
  });

  it('fails when gatekeeper injects PROTECTED_AREA_EXCEPTIONS_FILE via export', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: 'on:\n  pull_request:\njobs:\n  gatekeeper:\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      - run: export PROTECTED_AREA_EXCEPTIONS_FILE=./approval.json && npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: ${{ github.event.pull_request.number }}', contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
    });
    expect(failures.join('\n')).toContain('must not set protected-area exception env vars');
  });

  it('does not fail solely on protected-area exception env comment', () => {
    const failures = validateV3Gates({
      cwd: process.cwd(),
      packageJsonOverride: { scripts: { 'test:contracts': 'vitest run src/server/__tests__/v3-contracts.test.ts', 'gate:release': 'node scripts/validate-v3-gates.mjs && node scripts/validate-storage-bundle.mjs && node scripts/validate-protected-areas.mjs' } } as any,
      workflowOverride: { gatekeeper: 'on:\n  pull_request:\njobs:\n  gatekeeper:\n    steps:\n      - uses: actions/checkout@v4\n        with:\n          fetch-depth: 0\n      # PROTECTED_AREA_EXCEPTIONS_JSON\n      - run: npm run gate:release\n        env:\n          GITHUB_PR_NUMBER: ${{ github.event.pull_request.number }}', contracts: 'on:\n  pull_request:', build: 'on:\n  pull_request:' },
    });
