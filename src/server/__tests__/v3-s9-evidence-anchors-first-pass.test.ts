...
describe('v3 s9 evidence anchors first pass', () => {
  it('emits legacy timestamped notes as debug-only insufficient anchors', async () => {
    ...
    const out = await emitEvidenceAnchorsFirstPass({ run_id: run, analysis_run_id: run, submission_id: 'sub1', take_id: take, source_module: 'test', source_stage: 'unit', raw_report_data: { timestamped_notes: [{ timestamp: '00:12', note: 'Grounded beat lands.' }] }, root_dir: root, internal_qa_emit: true });
