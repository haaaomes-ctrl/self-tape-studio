...
} from "@/server/v3/qa-step2-dependency.server";
import {
...
async function emitLegacyBundle(options: LegacyBundleOptions = {}) {
  ...
  const analysisEvidenceStateOut = options.includeAnalysisEvidenceState
  ...
  await emitQAManifestForAnalysisRun({
    ...
    emitted_artefact_ids: [
      "raw_report",
      ...(analysisEvidenceStateOut?.emitted_artefact_ids ?? []),
      ...(anchorsOut.written ? ["evidence_anchors"] : []),
      ...(claimsOut.written ? ["public_claim_trace"] : []),
    ],
    emitted_blocked_artefact_ids: [
      ...(analysisEvidenceStateOut?.emitted_blocked_artefact_ids ?? []),
    ],
    artefact_source_classification_by_id: {
      raw_report: "legacy_adapter",
      ...(analysisEvidenceStateOut?.written
        ? { analysis_evidence_state: analysisEvidenceStateOut.source_classification }
        : {}),
    ...
    artefact_level2_spine_satisfaction_by_id: {
      raw_report: false,
      ...(analysisEvidenceStateOut?.written
        ? { analysis_evidence_state: analysisEvidenceStateOut.level2_satisfies }
        : {}),
    ...
    analysis_evidence_state_summary: analysisEvidenceStateOut?.written
      ? analysisEvidenceStateOut.summary
      : undefined,
  });
  const manifest = JSON.parse(await readFile(path.join(root, run, "manifest.json"), "utf8"));
  const metrics = JSON.parse(
    await readFile(path.join(root, run, "qa", "acceptance_metrics.json"), "utf8"),
  );
  const analysisEvidenceStatePath = path.join(
    root,
    run,
    "takes",
    `take-${take}`,
    `analysis-${run}`,
    "analysis",
    "AnalysisEvidenceState.json",
  );
  const analysisEvidenceState = analysisEvidenceStateOut?.written
    ? JSON.parse(await readFile(analysisEvidenceStatePath, "utf8"))
    : null;
  return {
    root,
    run,
...
async function emitAnalysisEvidenceStateBundle(
  options: {
    run?: string;
    take?: string;
    resolver?: boolean;
  ...
  const out = await emitAnalysisEvidenceStatePrerequisite({
    ...
    component_or_task_declaration_source:
      options.componentStatus === "supplied" ? "loaded_runtime_field" : "not_loaded",
    ...
    raw_report_data: {
      report_data: {
        timestamped_notes: [{ timestamp: "00:12", note: "Forbidden downstream note" }],
        fix_first: "Forbidden raw report field",
      },
    },
    root_dir: root,
    internal_qa_emit: true,
  ...
  await emitQAManifestForAnalysisRun({
    ...
    artefact_source_classification_by_id: out.written
      ? { analysis_evidence_state: out.source_classification }
      : {},
    artefact_level2_spine_satisfaction_by_id: out.written
      ? { analysis_evidence_state: out.level2_satisfies }
      : {},
...
async function emitPromotedEvidenceAnchorsBundle(
  options: {
  ...
  await emitQAManifestForAnalysisRun({
    ...
    evidence_anchor_trace_summary: anchorsOut.written
      ? anchorsOut.evidence_anchor_trace_summary
      : undefined,
  ...
  const manifest = JSON.parse(
    await readFile(path.join(base.root, base.run, "manifest.json"), "utf8"),
  );
  const metrics = JSON.parse(
    await readFile(path.join(base.root, base.run, "qa", "acceptance_metrics.json"), "utf8"),
  );
...
function sampleRunEvidencePass(overrides: Record<string, unknown> = {}) {
  return {
    ...
    detected_components: [
      { type: "monologue", weight: 1, score: 91, note: "report-ready component judgement" },
    ],
    raw_scores: {
      technical: 88,
      audio: 82,
    ...
    risk_evidence: [
      {
        severity: "medium",
        flag: "recall risk",
        why: "may reduce recall",
        recall_impact: "may_reduce",
      },
    ],
    timestamped_evidence: [
      {
        timestamp: "00:08",
        observation: "Performer shifts eyeline before the second phrase.",
        why_it_matters: "Useful moment",
        linked_category: "acting",
      },
      {
        timestamp: "00:14",
        observation: "Audio drops slightly on the final word.",
        why_it_matters: "Useful moment",
        linked_category: "audio",
      },
      {
        timestamp: "00:18",
        observation: "Supplied material text is visible in the runtime context.",
        why_it_matters: "Material context",
        linked_category: "brief_adherence",
      },
      {
        timestamp: "00:20",
        observation: "Ready to submit because the scene is strong.",
        why_it_matters: "Judgement",
        linked_category: "acting",
      },
    ],
...
async function emitFilteredRunEvidencePassAnalysisBundle(
  options: {
    evidence?: unknown;
    resolver?: boolean;
    truth?: boolean;
  ...
  const take = "t1";
  const filtered = filterRunEvidencePassForStep1(options.evidence ?? sampleRunEvidencePass(), {
    model: "test-model",
    durationSeconds: 60,
  });
  const out = await emitAnalysisEvidenceStatePrerequisite({
    ...
    raw_report_data: options.rawReport ?? {
      report_data: {
        timestamped_notes: [{ timestamp: "00:01", note: "Forbidden report note" }],
        strengths: ["Forbidden report strength"],
        fix_first: "Forbidden fix first",
        category_notes: { acting: "Forbidden category note" },
      },
    },
    root_dir: root,
    internal_qa_emit: true,
...
function completeAnalysisEvidenceStateForAggregate(
  payload: any,
  options: {
  ...
  const truthIdFor = (slug: string) =>
    `${payload.run_id}:truth_state:${slug
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "_")
      .replace(/^_+|_+$/g, "")}`;
  ...
  payload.observable_evidence_items = (payload.observable_evidence_items ?? []).map(
  ...
  if (options.materialNotApplicable) {
    ...
    addFamilyItem("material_specific_performance_evidence", {
      ...
      safe_evidence_summary:
        "A supplied material component is observed during the tape without judgement.",
      source_artefact_id: "run_evidence_pass",
  ...
  addFamilyItem("performance_observable_evidence", {
    ...
    safe_evidence_summary:
      "A pre-judgement observable performance event is recorded without score or verdict.",
    source_artefact_id: "run_evidence_pass",
  ...
  addFamilyItem("candidate_technique_evidence", {
    ...
    safe_evidence_summary:
      "Internal candidate technique observation recorded without public authority.",
    source_artefact_id: "run_evidence_pass",
  ...
  payload.observable_evidence_items = payload.observable_evidence_items.map(
    (item: any, index: number) => ({
      ...
      analysis_evidence_state_source_path:
        item.analysis_evidence_state_source_path ?? `observable_evidence_items[${index}]`,
    }),
  );
  payload.component_evidence = (payload.component_evidence ?? []).map((item: any) => ({
    ...
    linked_truth_state_ids: item.linked_truth_state_ids?.length
      ? item.linked_truth_state_ids
      : [truthIdFor("component_or_task_declaration_loaded")],
  }));
  payload.candidate_brief_evidence = (payload.candidate_brief_evidence ?? []).map((item: any) => ({
    ...item,
    linked_truth_state_ids: item.linked_truth_state_ids?.length
      ? item.linked_truth_state_ids
      : [truthIdFor(String(item.evidence_kind ?? "candidate_brief_fact"))],
  }));
  const truthIds = [
    ...payload.observable_evidence_items.flatMap((item: any) => item.linked_truth_state_ids ?? []),
    ...payload.step1_family_observable_evidence_items.flatMap(
      (item: any) => item.linked_truth_state_ids ?? [],
    ),
  ...
  payload.canonical_truth_state_ids = Object.fromEntries(
    payload.truth_state_ids.map((id: string) => [id.split(":truth_state:")[1] ?? id, id]),
  );
  ...
  payload.ordinary_analysis_family_completion_by_id = {
    ...
    video_observable: {
      ...
      blocker_codes:
        payload.evidence_family_coverage.video_observable === true
          ? []
          : ["missing_video_observable_evidence"],
    },
    audio_observable: {
      ...
      blocker_codes:
        payload.evidence_family_coverage.audio_observable === true
          ? []
          : ["missing_audio_observable_evidence"],
    },
    material_specific_performance: {
      ...
      can_satisfy_family_gate:
        payload.evidence_family_coverage.material_specific_performance === true,
      blocker_codes:
        payload.evidence_family_coverage.material_specific_performance === true ||
        payload.evidence_family_coverage.material_specific_performance === "not_applicable"
          ? []
    ...
    performance_observable: {
      ...
      blocker_codes:
        payload.evidence_family_coverage.performance_observable === true
          ? []
    ...
    candidate_technique: {
      ...
      blocker_codes:
        payload.evidence_family_coverage.candidate_technique === true
          ? []
          : ["missing_candidate_technique_evidence"],
    },
  };
}

async function emitAnchorsAndManifestFromAnalysisState(
  base: {
    root: string;
    run: string;
    take: string;
  ...
  await emitQAManifestForAnalysisRun({
    ...
    artefact_source_classification_by_id: {
      analysis_evidence_state:
        base.payload?.source_classification ?? base.out?.source_classification ?? "missing",
      ...(anchorsOut.written ? { evidence_anchors: anchorsOut.source_classification } : {}),
      ...(claimsOut?.written ? { public_claim_trace: "legacy_adapter" } : {}),
    },
    artefact_level2_spine_satisfaction_by_id: {
...
describe("S9-14A legacy containment and current-state guardrail", () => {
  it("keeps legacy EvidenceAnchors emitted but non-satisfying", async () => {
    ...
    expect(anchors.anchors.every((anchor: any) => anchor.cannot_satisfy_v3_gate === true)).toBe(
      true,
    );
    expect(anchors.blocker_codes).toEqual(
      expect.arrayContaining(["legacy_snapshot_insufficient_for_v3_evidence_anchor_gate"]),
    );
  });
  ...
  it("prevents raw_report-only EvidenceAnchors from becoming real_runtime_v3", async () => {
    ...
    expect(anchors.anchors.every((anchor: any) => anchor.source_artefact_id === "raw_report")).toBe(
      true,
    );
    expect(
      anchors.anchors.every((anchor: any) =>
        String(anchor.source_path).startsWith("report_data.timestamped_notes"),
      ),
    ).toBe(true);
    expect(manifest.artefact_source_classification_by_id.evidence_anchors).not.toBe(
      "real_runtime_v3",
    );
    expect(manifest.artefact_level2_spine_satisfaction_by_id.evidence_anchors).toBe(false);
  ...
  it("keeps EvidenceAnchors without TruthStateMap linkage non-satisfying", async () => {
    ...
    expect(anchors.anchors[0].blocker_codes).toEqual(
      expect.arrayContaining(["missing_truth_state_linkage"]),
    );
    expect(manifest.artefact_level2_spine_satisfaction_by_id.evidence_anchors).toBe(false);
    expect(metrics.evidence_anchor_gate_status).toBe("insufficient");
    expect(manifest.artefact_source_classification_by_id.evidence_anchors).not.toBe(
  ...
  it("keeps EvidenceAnchors without resolver or Step 1 provenance out of accepted gate evidence", async () => {
    ...
    expect(anchors.anchors[0].assessability_limitations).toContain(
      "legacy_report_snapshot_not_v3_multimodal",
    );
    expect(manifest.runtime_evidence_accepted_by_id).not.toContain("evidence_anchors");
  ...
  it("keeps legacy PublicClaimTrace emitted but non-satisfying", async () => {
    ...
    expect(metrics.public_claim_gate_reason).toBe(
  ...
  it("keeps public claims with no evidence anchor support blocked for rewrite", async () => {
    const { claims, metrics } = await emitLegacyBundle({
      report: { strengths: ["grounded acting"] },
    });
  ...
  it("keeps unsupported overclaims rewrite_required and unsupported", async () => {
    const { claims } = await emitLegacyBundle({
      report: { casting_headline: "Perfect match for this role" },
    });
    const claim = claims.claims.find(
      (entry: any) => entry.source_path === "report_data.casting_headline",
    );
  ...
  it("keeps score claims public_scoring_blocked", async () => {
    const { claims, metrics } = await emitLegacyBundle({
      report: { overall_score: 92, improvements: ["readiness score 92"] },
    });
    const overallScore = claims.claims.find(
      (entry: any) => entry.source_path === "report_data.overall_score",
    );
    const scoreWording = claims.claims.find(
      (entry: any) => entry.claim_text === "readiness score 92",
    );
  ...
  it("keeps public technique authority claims blocked from gate satisfaction", async () => {
    const { claims, metrics } = await emitLegacyBundle({
      report: { strengths: ["professional standard technique authority"] },
    });
    const claim = claims.claims.find(
      (entry: any) => entry.claim_text === "professional standard technique authority",
    );
  ...
  it("keeps castability bookability and marketability wording blocked or rewrite_required", async () => {
    const { claims, metrics } = await emitLegacyBundle({
      report: { casting_headline: "Bookable and marketable casting choice" },
    });
    const claim = claims.claims.find(
      (entry: any) => entry.source_path === "report_data.casting_headline",
    );
    expect(claim.rewrite_required).toBe(true);
    expect(claim.support_status).not.toBe("supported_by_evidence_anchor");
    expect(["needs_rewrite", "unsafe_or_overclaim", "blocked"]).toContain(
      claim.public_safety_status,
    );
    expect(metrics.public_claim_gate_status).toBe("insufficient");
  });
  ...
  it("keeps public comparison winner or recommendation wording blocked from public claim satisfaction", async () => {
    const { claims, metrics } = await emitLegacyBundle({
      report: { category_notes: ["Take 2 is the winner and recommended over Take 1"] },
    });
    const claim = claims.claims.find(
      (entry: any) => entry.claim_text === "Take 2 is the winner and recommended over Take 1",
    );
    expect(claim.rewrite_required).toBe(true);
    expect(claim.support_status).not.toBe("supported_by_evidence_anchor");
    expect(metrics.public_claim_gate_status).toBe("insufficient");
  ...
  it("does not treat emitted trace artefacts as satisfying or accepted gate evidence", async () => {
    ...
    expect(manifest.artefact_level2_spine_satisfaction_by_id.evidence_anchors).toBe(false);
    expect(manifest.artefact_level2_spine_satisfaction_by_id.public_claim_trace).toBe(false);
    expect(manifest.runtime_evidence_accepted_by_id).not.toEqual(
      expect.arrayContaining(["evidence_anchors", "public_claim_trace"]),
    );
    expect(metrics.runtime_evidence_accepted_by_id).not.toEqual(
      expect.arrayContaining(["evidence_anchors", "public_claim_trace"]),
    );
  });
  ...
  it("keeps malformed legacy anchors or claims from crashing finalisation", async () => {
    ...
    await emitQAManifestForAnalysisRun({
      ...
      artefact_source_classification_by_id: {
        evidence_anchors: "legacy_adapter",
        public_claim_trace: "legacy_adapter",
      },
      artefact_level2_spine_satisfaction_by_id: {
  ...
  it("prevents source_scaffold helper and local fixture classifications from satisfying gates", async () => {
    ...
    await emitQAManifestForAnalysisRun({
      ...
      artefact_source_classification_by_id: {
        evidence_anchors: "source_scaffold",
        public_claim_trace: "helper_test",
      },
      artefact_level2_spine_satisfaction_by_id: {
        evidence_anchors: true,
        public_claim_trace: true,
    ...
    expect(manifest.real_v3_spine_artefact_ids).not.toEqual(
      expect.arrayContaining(["evidence_anchors", "public_claim_trace"]),
    );
    expect(metrics.evidence_anchor_gate_status).toBe("insufficient");
    expect(metrics.public_claim_gate_status).toBe("insufficient");
...
describe("S9-14C Step 1 observable evidence extractor", () => {
  it("emits AnalysisEvidenceState from input resolver truth and media artefact refs", async () => {
    ...
    expect(payload.internal_only).toBe(true);
    expect(payload.privacy_classification).toBe("internal_private");
    expect(payload.input_artifact_refs.analysis_input_record).toContain(
    ...
    expect(manifest.artefact_source_classification_by_id.analysis_evidence_state).toBe(
      "real_runtime_v3",
    );
  ...
  it("does not derive AnalysisEvidenceState from raw_report or report_data snapshots", async () => {
    ...
    expect(
      payload.observable_evidence_items.some(
        (item: any) => item.source_artefact_id === "raw_report",
      ),
    ).toBe(false);
    expect(
      payload.observable_evidence_items.some((item: any) =>
        String(item.source_path ?? "").startsWith("report_data"),
      ),
    ).toBe(false);
  });
  ...
  it("records TruthStateMap known component and comparison truths with explicit canonical truth IDs", async () => {
    const { payload } = await emitAnalysisEvidenceStateBundle();
    const truthItems = payload.observable_evidence_items.filter(
      (item: any) => item.source_artefact_id === "truth_state_map",
    );
    expect(truthItems.map((item: any) => item.evidence_kind)).toEqual(
      expect.arrayContaining(["known_truths", "component_truths", "comparison_truths"]),
    );
    expect(
      truthItems.every(
        (item: any) =>
          Array.isArray(item.linked_truth_state_ids) &&
          item.linked_truth_state_ids.every((id: string) => id.includes(":truth_state:")),
      ),
    ).toBe(true);
    expect(payload.blocker_codes).not.toContain("structured_truth_state_ids_unavailable");
    expect(
      truthItems.some((item: any) =>
        item.assessability_limitations.includes(
  ...
  it("does not invent performance observations when no persisted extractor exists", async () => {
    ...
    expect(
      payload.observable_evidence_items.some((item: any) =>
        performanceKinds.includes(item.evidence_kind),
      ),
    ).toBe(false);
    expect(
      payload.unsupported_or_unavailable_evidence.map((item: any) => item.evidence_kind),
    ).toEqual(
  ...
  it("keeps candidate brief evidence to presence facts only", async () => {
    const { payload } = await emitAnalysisEvidenceStateBundle({
      briefPresence: "supplied",
      materialPresence: "supplied",
    });
  ...
  it("keeps candidate technique evidence unavailable when only legacy traces exist", async () => {
    ...
    expect(
      payload.unsupported_or_unavailable_evidence.map((item: any) => item.evidence_kind),
    ).toContain("candidate_technique_evidence_not_extracted");
    expect(metrics.public_technique_authority_status).toBe("blocked");
  });
  ...
  it("truthfully records unknown component or task declaration", async () => {
    ...
    expect(payload.component_evidence[0].blocker_codes).toContain(
  ...
  it("records selected_level audition_type and brief presence from runtime inputs or resolver", async () => {
    ...
    const selectedLevel = payload.observable_evidence_items.find(
      (item: any) => item.evidence_kind === "selected_level",
    );
    const auditionType = payload.observable_evidence_items.find(
      (item: any) => item.evidence_kind === "audition_type",
    );
    const briefPresence = payload.observable_evidence_items.find(
      (item: any) => item.evidence_kind === "brief_presence",
    );
    expect(selectedLevel).toMatchObject({
      source_artefact_id: "analysis_submission",
      source_path: "selected_level",
    });
    expect(auditionType).toMatchObject({
      source_artefact_id: "analysis_submission",
      source_path: "audition_type",
    });
    expect(briefPresence).toMatchObject({
  ...
  it("records stable take identity from analysis_take without unsafe media refs", async () => {
    const { payload } = await emitAnalysisEvidenceStateBundle();
    const identity = payload.observable_evidence_items.find(
      (item: any) => item.evidence_kind === "stable_take_identity",
    );
  ...
  it("records partial real_runtime_v3 runtime facts but keeps the Step 1 gate non-satisfying", async () => {
    ...
    expect(metrics.analysis_evidence_state_gate_reason).toBe(
  ...
  it("blocks satisfaction when TruthStateMap is missing", async () => {
    ...
    expect(
      payload.observable_evidence_items.some((item: any) =>
        item.blocker_codes.includes("TruthStateMap_missing"),
      ),
    ).toBe(true);
    expect(metrics.analysis_evidence_state_gate_status).toBe("insufficient");
  });
  ...
  it("truthfully records missing media readiness duration without padded timestamps", async () => {
    ...
    expect(payload.assessability_limitations).toContain(
      "media_duration_unavailable_no_timestamp_evidence_fabricated",
    );
    expect(
      payload.observable_evidence_items.some((item: any) =>
        item.blocker_codes.includes("media_duration_unavailable"),
      ),
    ).toBe(true);
  });
  ...
  it("does not crash on partial prerequisite input and preserves blockers", async () => {
    const { out, payload, manifest } = await emitAnalysisEvidenceStateBundle({
      run: "run-s914b-malformed",
      take: "t1",
      resolver: false,
      truth: false,
      duration: null,
    ...
    expect(payload.blocker_codes).toEqual(
      expect.arrayContaining([
  ...
  it("keeps PublicClaimTrace legacy and non-satisfying after AnalysisEvidenceState emits", async () => {
    const { claims, manifest, metrics } = await emitLegacyBundle({
      includeAnalysisEvidenceState: true,
    });
    expect(manifest.artefact_source_classification_by_id.public_claim_trace).toBe("legacy_adapter");
    expect(manifest.artefact_level2_spine_satisfaction_by_id.public_claim_trace).toBe(false);
    expect(claims.cannot_satisfy_public_claim_gate).toBe(true);
  ...
  it("aligns manifest and qa_acceptance_metrics for AnalysisEvidenceState status and blockers", async () => {
    ...
    expect(metrics.analysis_evidence_state_status).toBe(
      manifest.artefact_status_by_id.analysis_evidence_state,
    );
    expect(metrics.analysis_evidence_state_source_classification).toBe(
      manifest.artefact_source_classification_by_id.analysis_evidence_state,
    );
    expect(metrics.analysis_evidence_state_gate_status).toBe("insufficient");
    expect(metrics.blocker_codes).toEqual(manifest.blocker_codes);
...
describe("S9-14D EvidenceAnchors real_runtime_v3 promotion", () => {
  it("promotes AnalysisEvidenceState observable evidence items to real_runtime_v3 anchors", async () => {
    ...
    const selectedLevel = anchors.anchors.find(
      (anchor: any) => anchor.evidence_text === "selected_level: advanced",
    );
    expect(selectedLevel).toMatchObject({
    ...
    expect(payload.observable_evidence_items[0].safe_evidence_summary).toBe(
      selectedLevel.evidence_text,
    );
  });
  ...
  it("promotes selected_level and audition_type as factual deterministic anchors", async () => {
    ...
    expect(texts).toEqual(
      expect.arrayContaining(["selected_level: advanced", "audition_type: monologue"]),
    );
    ...
    const selectedAndAudition = anchors.anchors.filter((anchor: any) =>
      ["selected_level: advanced", "audition_type: monologue"].includes(anchor.evidence_text),
    );
  ...
  it("promotes stable take identity and media readiness facts with safe refs only", async () => {
    const { anchors } = await emitPromotedEvidenceAnchorsBundle();
    const identity = anchors.anchors.find((anchor: any) =>
      anchor.evidence_text.includes("stable_take_identity"),
    );
    const media = anchors.anchors.find((anchor: any) =>
      anchor.evidence_text.includes("media_readiness_state"),
    );
    expect(identity).toMatchObject({
      source_artefact_id: "analysis_evidence_state",
      source_family: "real_runtime_v3",
  ...
  it("promotes brief and material presence without achievement or suitability claims", async () => {
    const { anchors } = await emitPromotedEvidenceAnchorsBundle();
    const briefAnchors = anchors.anchors.filter((anchor: any) =>
      String(anchor.evidence_text).includes("presence"),
    );
  ...
  it("promotes component declaration status as a fact or limitation only", async () => {
    const { anchors } = await emitPromotedEvidenceAnchorsBundle();
    const component = anchors.anchors.find(
      (anchor: any) => anchor.source_path === "component_evidence[0]",
    );
  ...
  it("promotes TruthStateMap anchors when explicit truth IDs are available", async () => {
    const { anchors } = await emitPromotedEvidenceAnchorsBundle();
    const truthAnchors = anchors.anchors.filter(
      (anchor: any) => anchor.evidence_modality === "resolver_truth",
    );
    expect(truthAnchors.length).toBeGreaterThan(0);
    expect(
      truthAnchors.every((anchor: any) => anchor.source_artefact_id === "analysis_evidence_state"),
    ).toBe(true);
    expect(truthAnchors.every((anchor: any) => anchor.cannot_satisfy_v3_gate === false)).toBe(true);
    expect(
      truthAnchors.every((anchor: any) =>
        anchor.linked_truth_state_ids.every((id: string) => id.includes(":truth_state:")),
      ),
    ).toBe(true);
  });
  ...
  it("keeps raw_report timestamped notes as excluded legacy diagnostics beside promoted anchors", async () => {
    const { anchors, manifest, metrics } = await emitPromotedEvidenceAnchorsBundle({
      rawReport: {
        report_data: { timestamped_notes: [{ timestamp: "00:10", note: "Legacy report note" }] },
      },
    });
    const legacy = anchors.legacy_diagnostic_anchors.find(
      (anchor: any) => anchor.source_artefact_id === "raw_report",
    );
    ...
    expect(
      anchors.anchors.some(
        (anchor: any) =>
          anchor.source_artefact_id === "raw_report" &&
          anchor.source_classification === "real_runtime_v3",
      ),
    ).toBe(false);
    expect(manifest.artefact_source_classification_by_id.evidence_anchors).toBe(
  ...
  it("does not promote a legacy report snapshot because public_safe is true", async () => {
    const { anchors } = await emitPromotedEvidenceAnchorsBundle({
      rawReport: {
        report_data: {
          timestamped_notes: [
            { timestamp: "00:10", note: "Legacy public-safe note", public_safe: true },
          ],
        },
      },
    });
    const legacy = anchors.legacy_diagnostic_anchors.find(
      (anchor: any) => anchor.evidence_text === "Legacy public-safe note",
    );
  ...
  it("blocks promotion when the AnalysisEvidenceState source path is unresolved", async () => {
    const { anchors } = await emitPromotedEvidenceAnchorsBundle({
      transformAnalysisEvidenceState(payload) {
        payload.observable_evidence_items[0].analysis_evidence_state_source_path =
          "observable_evidence_items[99]";
      },
    });
    const blocked = anchors.anchors.find(
      (anchor: any) => anchor.source_path === "observable_evidence_items[99]",
    );
    ...
    const { anchors } = await emitPromotedEvidenceAnchorsBundle({
      transformAnalysisEvidenceState(payload) {
        ...
        payload.observable_evidence_items = payload.observable_evidence_items.map((item: any) =>
          item.evidence_modality === "resolver_truth"
            ? { ...item, linked_truth_state_ids: [] }
            : item,
        );
      },
    });
    const knownTruths = anchors.anchors.find(
      (anchor: any) =>
        anchor.evidence_modality === "resolver_truth" &&
        anchor.evidence_text.includes("known runtime truth"),
    );
  ...
  it("blocks fabricated evidence text that is not supported by the source path", async () => {
    const { anchors } = await emitPromotedEvidenceAnchorsBundle({
      transformAnalysisEvidenceState(payload) {
        payload.observable_evidence_items[0].safe_evidence_summary =
          "fabricated unsupported claim from nowhere";
        payload.observable_evidence_items[0].source_artefact_id = "raw_report";
      },
    });
    const fabricated = anchors.anchors.find(
      (anchor: any) => anchor.evidence_text === "fabricated unsupported claim from nowhere",
    );
  ...
  it("does not use legacy TechniqueObservationTrace for candidate technique anchors", async () => {
    ...
    expect(
      anchors.anchors.some(
        (anchor: any) => anchor.source_artefact_id === "technique_observation_trace",
      ),
    ).toBe(false);
    expect(metrics.public_technique_authority_status).toBe("blocked");
  });
  ...
  it("aligns manifest and qa_acceptance_metrics for deterministic anchor promotion", async () => {
    ...
    expect(metrics.evidence_anchor_trace_status).toBe(
      manifest.artefact_status_by_id.evidence_anchors,
    );
    expect(metrics.evidence_anchor_source_family_summary).toEqual(
      anchors.evidence_anchor_trace_summary.source_family_summary,
    );
    expect(metrics.evidence_anchor_gate_status).toBe("insufficient");
    expect(metrics.blocker_codes).toEqual(manifest.blocker_codes);
  });
  ...
  it("keeps PublicClaimTrace legacy and non-satisfying after EvidenceAnchors promotion", async () => {
    const { claims, manifest, metrics } = await emitPromotedEvidenceAnchorsBundle({
  ...
  it("does not crash on malformed AnalysisEvidenceState input", async () => {
    ...
    const out = await emitEvidenceAnchorsFirstPass({
      ...
      analysis_evidence_state_data: {
        run_id: "run-s914d-malformed",
        analysis_run_id: "run-s914d-malformed",
        observable_evidence_items: [null, {}, { safe_evidence_summary: "" }],
      },
      root_dir: root,
      internal_qa_emit: true,
    } as any);
    ...
    const evidenceAnchorSummary = expectPresent(
      out.evidence_anchor_trace_summary,
      "evidence anchor trace summary",
    );
...
describe("S9-14G hardened runEvidencePass persisted Step 1 extractor", () => {
  it("persists filtered runEvidencePass output as Step 1 before Step 2/raw_report", async () => {
    ...
    expect(payload.step2_dependency_status).toMatchObject({
  ...
  it("filters score fields and preserves public scoring as blocked", async () => {
    ...
    expect(payload.rejected_or_filtered_fields).toEqual(
      expect.arrayContaining([
        "overall_score",
        "score",
        "score_breakdown",
    ...
    const { payload, metrics } = await emitFilteredRunEvidencePassAnalysisBundle({
      evidence: sampleRunEvidencePass({
        candidate_technique_evidence: [
          {
            safe_evidence_summary: "Breath support pattern is observable as a candidate only.",
            label: "breath_support",
          },
          {
            safe_evidence_summary: "Authoritative diagnosis with score",
            score: 99,
            authoritative: true,
          },
        ],
  ...
  it("persists video, audio, material and performance observations when provided by runEvidencePass", async () => {
    const { payload } = await emitFilteredRunEvidencePassAnalysisBundle();
    expect(
      payload.video_observable_evidence_items.some(
        (item: any) => item.source_artefact_id === "run_evidence_pass",
      ),
    ).toBe(true);
    expect(
      payload.audio_observable_evidence_items.some((item: any) => item.evidence_family === "audio"),
    ).toBe(true);
    expect(
      payload.material_observable_evidence_items.some(
        (item: any) => item.evidence_family === "material",
      ),
    ).toBe(true);
    expect(
      payload.performance_observable_evidence_items.some(
        (item: any) => item.evidence_family === "performance",
      ),
    ).toBe(true);
  ...
  it("preserves explicit valid modality on performance Step 1 evidence", () => {
    const filtered = filterRunEvidencePassForStep1(
      sampleRunEvidencePass({
        timestamped_evidence: [
          {
            timestamp: "00:08",
            observation: "Breath is audible before the phrase.",
            linked_category: "acting",
            evidence_modality: "audio",
          },
          {
            timestamp: "00:12",
            observation: "Physical shift is visible before the turn.",
            linked_category: "acting",
            modality: "video",
          },
        ],
    ...
    expect(
      filtered.performance_observable_evidence_items.map((item) => item.evidence_family),
    ).toEqual(["performance", "performance"]);
    expect(
      filtered.performance_observable_evidence_items.map((item) => item.evidence_modality),
    ).toEqual(["audio", "video"]);
    expect(
      filtered.performance_observable_evidence_items.map((item) => item.evidence_modality),
  ...
  it("keeps candidate technique evidence on a valid modality enum", () => {
    ...
    expect(filtered.candidate_technique_evidence.map((item) => item.evidence_modality)).toEqual([
      "video",
      "unknown",
    ]);
    expect(
      filtered.candidate_technique_evidence.map((item) => item.evidence_modality),
    ).not.toContain("candidate_technique");
    expect(
      filtered.candidate_technique_evidence.map((item) => item.evidence_modality),
    ).not.toContain("performance");
  });
  ...
  it("does not hand downstream AnalysisEvidenceState an invalid performance modality", async () => {
    ...
    const performanceModalities = payload.performance_observable_evidence_items.map(
      (item: any) => item.evidence_modality,
    );
    ...
    expect(
      payload.observable_evidence_items.every((item: any) =>
        allowed.includes(item.evidence_modality),
      ),
    ).toBe(true);
  });
  ...
  it("records missing extractor families as unavailable instead of fabricating evidence", async () => {
    ...
    expect(payload.evidence_state_status).toBe("partial");
    expect(payload.cannot_satisfy_v3_gate).toBe(true);
    const unavailableKinds = payload.unsupported_or_unavailable_evidence.map(
      (item: any) => item.evidence_kind,
    );
  ...
  it("reports Step 1 write failure without creating satisfying evidence", async () => {
    const rootFile = path.join(
      os.tmpdir(),
      `qa-s914g-not-dir-${Math.random().toString(36).slice(2)}`,
    );
  ...
  it("allows Step 2 to continue when Step 1 evidence is valid but the QA sink write fails", async () => {
    const rootFile = path.join(
      os.tmpdir(),
      `qa-s914g-step2-sink-fail-${Math.random().toString(36).slice(2)}`,
    );
    await writeFile(rootFile, "not a directory", "utf8");
    ...
    const manifestRoot = await mkdtemp(
      path.join(os.tmpdir(), "qa-s914g-step2-sink-fail-manifest-"),
    );
    await emitQAManifestForAnalysisRun({
      run_id: run,
      analysis_run_id: run,
    ...
    const manifest = JSON.parse(
      await readFile(path.join(manifestRoot, run, "manifest.json"), "utf8"),
    );
    const metrics = JSON.parse(
      await readFile(path.join(manifestRoot, run, "qa/acceptance_metrics.json"), "utf8"),
    );
    expect(manifest.artefact_status_by_id.analysis_evidence_state).toBe("emitted_blocked");
    expect(metrics.analysis_evidence_state_status).toBe("emitted_blocked");
    expect(metrics.level2_status).toBe("not_accepted");
  ...
  it("blocks Step 2 when Step 1 evidence is invalid even if a result object exists", async () => {
    ...
    expect(dependency.step1EvidenceValidForStep2).toBe(false);
    expect(dependency.step2DependencyBlocked).toBe(true);
    expect(dependency.blockerCodes).toEqual(
      expect.arrayContaining([
  ...
  it("handles malformed runEvidencePass output without crashing", async () => {
    const { payload, out } = await emitFilteredRunEvidencePassAnalysisBundle({
      evidence: "malformed-runEvidencePass-output",
    });
    expect(out.written).toBe(true);
  ...
  it("does not let caller metadata override canonical AnalysisEvidenceState fields", async () => {
    ...
    const filtered = {
      ...filterRunEvidencePassForStep1(sampleRunEvidencePass(), {
        model: "test-model",
        durationSeconds: 60,
      }),
      schema_version: "attacker",
  ...
  it("aligns manifest and qa_acceptance_metrics for AnalysisEvidenceState and keeps global gates blocked", async () => {
    ...
    expect(manifest.artefact_status_by_id.analysis_evidence_state).toBe(
      metrics.analysis_evidence_state_status,
    );
    expect(manifest.artefact_source_classification_by_id.analysis_evidence_state).toBe(
      metrics.analysis_evidence_state_source_classification,
    );
    expect(manifest.artefact_level2_spine_satisfaction_by_id.analysis_evidence_state).toBe(false);
    expect(metrics.analysis_evidence_state_gate_status).toBe("insufficient");
    const analysisSummary = expectPresent(out.summary, "analysis evidence summary");
    expect(metrics.analysis_evidence_state_gate_reason).toBe(
      analysisSummary.analysis_evidence_state_gate_reason,
    );
    expect(metrics.blocker_codes).toEqual(manifest.blocker_codes);
...
describe("S9-14H EvidenceAnchors aggregate promotion audit", () => {
  it("keeps current partial S9-14G coverage aggregate insufficient while individual anchors are real_runtime_v3", async () => {
    ...
    expect(anchors.real_runtime_anchor_count).toBeGreaterThan(0);
    expect(anchors.anchors.some((anchor: any) => anchor.source_family === "real_runtime_v3")).toBe(
      true,
    );
    expect(anchors.evidence_anchor_trace_summary.evidence_anchor_gate_status).toBe("insufficient");
    expect(anchors.evidence_anchor_trace_summary.blocker_codes).toEqual(
      expect.arrayContaining(["missing_candidate_technique_evidence"]),
    );
    expect(anchors.evidence_anchor_trace_summary.blocker_codes).not.toContain(
  ...
  it("allows complete required evidence families to satisfy the EvidenceAnchors subgate only", async () => {
    const bundle = await emitAnalysisEvidenceStateBundle({
      duration: 42,
      componentStatus: "supplied",
    });
    ...
    expect(
      anchors.anchors.every(
        (anchor: any) => anchor.source_artefact_id === "analysis_evidence_state",
      ),
    ).toBe(true);
    expect(anchors.anchors.every((anchor: any) => anchor.source_family === "real_runtime_v3")).toBe(
      true,
    );
  ...
  it("keeps unavailable video audio material and candidate technique families aggregate insufficient when required", async () => {
    for (const [family, expectedReason] of [
      ...
      const { anchors, metrics } = await emitAnchorsAndManifestFromAnalysisState(bundle);
      expect(anchors.evidence_anchor_trace_summary.evidence_anchor_gate_status).toBe(
        "insufficient",
      );
  ...
  it("keeps legacy timestamped notes diagnostic without blocking complete real_runtime_v3 anchors", async () => {
    ...
    expect(anchors.evidence_anchor_trace_summary.evidence_anchor_gate_status).toBe("sufficient");
    expect(anchors.blocker_codes).not.toEqual(
      expect.arrayContaining([
    ...
    expect(anchors.legacy_diagnostic_anchors).toHaveLength(1);
    expect(anchors.legacy_diagnostic_anchors[0]).toMatchObject({
      source_family: "legacy_adapter",
      excluded_from_evidence_anchor_gate: true,
    });
    expect(manifest.artefact_source_classification_by_id.evidence_anchors).toBe("real_runtime_v3");
  ...
  it("does not count raw_report or report_data anchors toward aggregate satisfaction", async () => {
    ...
    const { anchors, metrics } = await emitAnchorsAndManifestFromAnalysisState(bundle, {
      rawReport: {
        report_data: {
          timestamped_notes: [{ timestamp: "00:10", note: "Legacy report_data note" }],
        },
      },
    });
    const legacy = anchors.legacy_diagnostic_anchors.find(
    ...
    expect(legacy.excluded_from_evidence_anchor_gate).toBe(true);
    expect(metrics.evidence_anchor_gate_status).toBe("sufficient");
    expect(metrics.evidence_anchor_gate_reason).toBe(
  ...
  it("blocks aggregate satisfaction on unresolved AnalysisEvidenceState source paths", async () => {
    ...
    completeAnalysisEvidenceStateForAggregate(bundle.payload);
    bundle.payload.performance_observable_evidence[0].analysis_evidence_state_source_path =
      "performance_observable_evidence[999]";
    const { anchors, metrics } = await emitAnchorsAndManifestFromAnalysisState(bundle);
    expect(anchors.evidence_anchor_trace_summary.evidence_anchor_gate_status).toBe("insufficient");
  ...
  it("does not count anchors whose cannot_satisfy_v3_gate remains true", async () => {
    ...
    completeAnalysisEvidenceStateForAggregate(bundle.payload);
    bundle.payload.performance_observable_evidence[0].blocker_codes = [
      "caller_marked_anchor_blocked",
    ];
    const { anchors, metrics } = await emitAnchorsAndManifestFromAnalysisState(bundle);
    expect(anchors.evidence_anchor_trace_summary.evidence_anchor_gate_status).toBe("insufficient");
  ...
  it("does not count rejected prohibited runEvidencePass fields as evidence coverage", async () => {
    ...
    bundle.payload.rejected_or_filtered_fields = [
      "overall_score",
      "fix_first",
      "role_fit_evidence",
    ];
    bundle.payload.prohibited_field_filter_summary = {
      rejected_field_count: 3,
      rejected_field_keys: ["overall_score", "fix_first", "role_fit_evidence"],
  ...
  it("aligns manifest and qa_acceptance_metrics for sufficient and insufficient aggregate cases", async () => {
    const complete = await emitAnalysisEvidenceStateBundle({
      duration: 42,
      componentStatus: "supplied",
    });
    ...
    expect(sufficient.manifest.artefact_level2_spine_satisfaction_by_id.evidence_anchors).toBe(
      true,
    );
    expect(sufficient.metrics.evidence_anchor_gate_status).toBe("sufficient");
    expect(sufficient.metrics.evidence_anchor_source_family_summary).toEqual(
      sufficient.anchors.evidence_anchor_trace_summary.source_family_summary,
    );
  ...
  it("keeps PublicClaimTrace legacy and global accepted gate posture blocked", async () => {
    ...
    completeAnalysisEvidenceStateForAggregate(bundle.payload);
    const { claims, manifest, metrics } = await emitAnchorsAndManifestFromAnalysisState(bundle, {
      includePublicClaimTrace: true,
    });
    expect(manifest.artefact_source_classification_by_id.public_claim_trace).toBe("legacy_adapter");
    expect(claims.cannot_satisfy_public_claim_gate).toBe(true);
    ...
    expect(metrics.public_scoring_status).toBe("blocked");
    expect(metrics.public_technique_authority_status).toBe("blocked");
    expect(JSON.stringify(manifest.artefact_source_classification_by_id)).not.toContain(
...
async function emitClaimCandidateBundle(
  options: {
    rawReport?: Record<string, unknown> | null;
  ...
  const claimCandidatePath = path.join(
    base.root,
    base.run,
    "takes",
    `take-${base.take}`,
    `analysis-${base.run}`,
    "traces",
    "ClaimCandidateTrace.json",
  );
  const claimCandidateTrace = claimCandidateOut.written
    ? JSON.parse(await readFile(claimCandidatePath, "utf8"))
    : null;
  ...
  await emitQAManifestForAnalysisRun({
    ...
    artefact_source_classification_by_id: {
      ...
      ...(anchorsOut?.written ? { evidence_anchors: anchorsOut.source_classification } : {}),
      ...(claimCandidateOut.written
        ? { claim_candidate_trace: claimCandidateOut.source_classification }
        : {}),
      ...(claimsOut?.written ? { public_claim_trace: "legacy_adapter" } : {}),
    },
    artefact_level2_spine_satisfaction_by_id: {
...
describe("S9-14K v3 ClaimCandidate artefact", () => {
  ...
  it("does not let caller metadata override canonical ClaimCandidateTrace fields", async () => {
    ...
    expect(claimCandidateTrace.privacy_classification).toBe("internal_private");
    expect(claimCandidateTrace.cannot_satisfy_public_claim_gate).toBe(false);
    expect(claimCandidateTrace.blocker_codes).not.toContain(
  ...
  it("creates factual status candidates from AnalysisEvidenceState without rendering them", async () => {
    ...
    const selectedLevel = claimCandidateTrace.claim_candidates.find((candidate: any) =>
      String(candidate.safe_candidate_summary).includes("selected_level"),
    );
    const mediaReadiness = claimCandidateTrace.claim_candidates.find((candidate: any) =>
      String(candidate.safe_candidate_summary).includes("media_readiness_state"),
    );
    expect(selectedLevel).toMatchObject({
      source_artefact_id: "analysis_evidence_state",
      source_family: "real_runtime_v3",
  ...
  it("creates assessability limitation candidates from Step 1 unavailable evidence families", async () => {
    const { claimCandidateTrace } = await emitClaimCandidateBundle();
    const limitation = claimCandidateTrace.claim_candidates.find(
      (candidate: any) =>
    ...
    expect(["safe_for_public_candidate", "needs_rewrite"]).toContain(
      limitation.public_safety_status,
    );
  });
  ...
  it("creates limitation candidates from string record and mixed Step 1 assessability limitations", async () => {
    ...
    const byPath = new Map(
      claimCandidateTrace.claim_candidates.map((candidate: any) => [
  ...
  it("redacts unsafe string limitations instead of leaking raw diagnostics", async () => {
    ...
    const limitation = claimCandidateTrace.claim_candidates.find(
      (candidate: any) => candidate.source_path === "assessability_limitations[0]",
    );
  ...
  it("preserves string unavailable-evidence and timestamp-warning limitations as candidates", async () => {
    ...
    const unavailable = claimCandidateTrace.claim_candidates.find(
      (candidate: any) => candidate.source_path === "unsupported_or_unavailable_evidence[0]",
    );
    const timestampWarning = claimCandidateTrace.claim_candidates.find(
      (candidate: any) => candidate.source_path === "timestamp_normalisation_warnings[0]",
    );
    const timestampWarningAlias = claimCandidateTrace.claim_candidates.find(
      (candidate: any) => candidate.source_path === "timestamp_normalization_warnings[0]",
    );
  ...
  it("records raw_report submission verdict candidates as legacy_or_unsupported", async () => {
    const { claimCandidateTrace } = await emitClaimCandidateBundle({
      ...
      rawReport: {
        submission_verdict: { label: "Submit", reason: "Ready to submit with confidence" },
      },
    });
    expect(claimCandidateTrace.source_classification).toBe("legacy_or_unsupported");
    const candidate = claimCandidateTrace.claim_candidates.find(
      (item: any) => item.source_path === "report_data.submission_verdict.label",
    );
  ...
  it("keeps raw_report fix_first next_take strengths and category notes non-satisfying", async () => {
    ...
    const legacyCandidates = claimCandidateTrace.claim_candidates.filter((item: any) =>
      String(item.source_path).startsWith("report_data."),
    );
    expect(legacyCandidates.length).toBeGreaterThanOrEqual(4);
    expect(legacyCandidates.every((item: any) => item.source_family === "legacy_adapter")).toBe(
      true,
    );
    expect(
      legacyCandidates.every(
        (item: any) => item.candidate_support_precheck_status === "legacy_diagnostic_only",
      ),
    ).toBe(true);
    expect(
      legacyCandidates.every((item: any) => item.required_for_public_claim_gate === false),
    ).toBe(true);
    expect(
      legacyCandidates.every((item: any) => item.excluded_from_public_claim_gate === true),
    ).toBe(true);
  });
  ...
  it("blocks score candidates and keeps public scoring blocked", async () => {
    ...
    const scoreCandidates = claimCandidateTrace.claim_candidates.filter(
      (candidate: any) => candidate.blocked_claim_category === "public_scoring",
    );
    expect(scoreCandidates.length).toBeGreaterThan(0);
    expect(
      scoreCandidates.every((candidate: any) =>
        candidate.blocker_codes.includes("public_scoring_blocked"),
      ),
    ).toBe(true);
  ...
  it.each([
    [
      "technique authority",
      { category_notes: ["Meisner technique authority diagnosis says this is ready"] },
      "public_technique_authority",
    ],
    [
      "castability bookability marketability",
      { casting_headline: "High castability and marketability for this role" },
      "castability_bookability_marketability",
    ],
    [
      "public comparison winner",
      { category_notes: ["Take 2 is the winner and recommended over Take 1"] },
    ...
    const blocked = claimCandidateTrace.claim_candidates.find(
      (candidate: any) => candidate.blocked_claim_category === blockedCategory,
    );
  ...
  it("marks role and brief-fit overclaim candidates as rewrite_required", async () => {
    const { claimCandidateTrace } = await emitClaimCandidateBundle({
      includeAnalysis: false,
      rawReport: {
        category_notes: ["This fits the brief perfectly and should be sent with confidence"],
      },
    });
  ...
  it("aligns manifest and qa_acceptance_metrics claim candidate summaries", async () => {
    ...
    expect(manifest.artefact_source_classification_by_id.claim_candidate_trace).toBe(
      claimCandidateTrace.source_classification,
    );
    expect(metrics.claim_candidate_trace_status).toBe("emitted");
    expect(metrics.claim_candidate_source_classification).toBe(
      claimCandidateTrace.source_classification,
    );
    expect(metrics.claim_candidate_trace_summary.claim_candidate_count).toBe(
      claimCandidateTrace.claim_candidate_count,
    );
    expect(metrics.claim_candidate_source_summary).toEqual(
      claimCandidateTrace.claim_candidate_source_summary,
    );
    expect(metrics.blocker_codes).not.toContain(
      "claim_candidate_trace_internal_only_not_public_claim_gate_evidence",
    );
  });
  ...
  it("does not crash on malformed claim source input and preserves non-satisfying status", async () => {
    const { claimCandidateTrace, metrics } = await emitClaimCandidateBundle({
      includeAnalysis: false,
      rawReport: {
        fix_first: { malformed: true },
        strengths: [{ nested: ["bad"] }, "Usable legacy strength"],
      },
    });
...
async function emitPublicClaimSupportBundle(
  options: {
  ...
  const evidenceAnchorsData = {
    ...
    evidence_anchor_gate_reason:
      evidenceAnchorGateStatus === "sufficient"
        ? "complete_controlled_fixture"
        : "partial_step1_evidence_coverage",
    evidence_anchor_source_family_summary: {
      real_runtime_v3: anchors.filter(
        (anchor) =>
      ...
      report_snapshot: 0,
      source_scaffold: anchors.filter((anchor) => anchor.source_family === "source_scaffold")
        .length,
    },
    ...
    blocker_codes:
      evidenceAnchorGateStatus === "sufficient" ? [] : ["partial_step1_evidence_coverage"],
    ...
    evidence_anchor_trace_summary: {
      ...
      source_family_summary: {
        ...
        legacy_adapter: anchors.filter((anchor) => anchor.source_family === "legacy_adapter")
          .length,
        report_snapshot: 0,
        source_scaffold: anchors.filter((anchor) => anchor.source_family === "source_scaffold")
          .length,
  ...
  const claimCandidateTrace = {
    ...
    internal_only: true,
    privacy_classification: "internal_private",
    source_classification: candidates.some(
      (candidate) => candidate.source_family === "legacy_adapter",
    )
      ? "mixed_real_runtime_v3_and_legacy_or_unsupported"
      : "real_runtime_v3_candidate_source",
    claim_candidate_count: candidates.length,
    claim_candidates: candidates,
    claim_candidate_source_summary: candidates.reduce<Record<string, number>>(
      (acc, candidate) => {
  ...
  const claimsOut = await emitPublicClaimTraceFirstPass({
    ...
    truth_state_map_data:
      options.truthStateMap === undefined
        ? {
            run_id: run,
            analysis_run_id: run,
            take_id: take,
            truth_state_ids: ["truth-brief-presence"],
            known_truths: { "truth-brief-presence": "supplied" },
          }
        : options.truthStateMap,
    metadata_overrides: options.metadataOverrides,
    root_dir: root,
    internal_qa_emit: true,
  });
  ...
  await emitQAManifestForAnalysisRun({
    ...
    legacy_adapter_artefact_ids:
      claims?.source_classification === "legacy_or_unsupported" ? ["public_claim_trace"] : [],
    real_v3_spine_artefact_ids: claimsOut.level2_satisfies
      ? ["evidence_anchors", "public_claim_trace"]
      : evidenceAnchorGateStatus === "sufficient"
        ? ["evidence_anchors"]
        : [],
    runtime_evidence_accepted_by_id: claimsOut.level2_satisfies
      ? ["evidence_anchors", "public_claim_trace"]
      : evidenceAnchorGateStatus === "sufficient"
        ? ["evidence_anchors"]
        : [],
...
describe("S9-14L PublicClaimTrace support classification", () => {
  ...
  it.each([
    [
      "submission verdict",
      claimCandidate({
        safe_candidate_summary: "Submit this take with a 95 score",
        claim_type: "score_or_verdict",
        claim_family: "readiness_status",
        source_artefact_id: "raw_report",
        source_path: "report_data.submission_verdict.label",
        source_family: "legacy_adapter",
    ...
    [
      "fix_first next_take",
      claimCandidate({
        safe_candidate_summary: "Fix first: make the next take sharper",
        claim_type: "priority_fix",
        claim_family: "priority_fix",
        source_artefact_id: "raw_report",
        source_path: "report_data.fix_first",
        source_family: "legacy_adapter",
        linked_evidence_anchor_ids: [],
      }),
      "legacy_or_unsupported",
    ...
    [
      "category notes strengths",
      claimCandidate({
        safe_candidate_summary: "Strong acting presence",
        claim_type: "preserve_strength",
        claim_family: "preserve_strength",
        source_artefact_id: "raw_report",
        source_path: "report_data.strengths[0]",
        source_family: "legacy_adapter",
        linked_evidence_anchor_ids: [],
      }),
      "legacy_or_unsupported",
  ...
  it.each([
    [
      "score",
      claimCandidate({
        safe_candidate_summary: "overall score: 94",
        claim_type: "score_or_verdict",
        claim_family: "score_or_verdict",
    ...
    [
      ...
      claimCandidate({
        safe_candidate_summary: "Meisner technique authority diagnosis",
        claim_type: "technique_authority",
        claim_family: "technique_authority",
    ...
    [
      "castability bookability marketability",
      claimCandidate({
        safe_candidate_summary: "High castability and marketability",
        claim_type: "castability_bookability_marketability",
        claim_family: "castability_bookability_marketability",
        blocked_claim_category: "castability_bookability_marketability",
        blocker_codes: ["castability_bookability_marketability_blocked"],
      }),
      "castability_bookability_marketability",
      "castability_bookability_marketability_blocked",
    ],
    [
      "public comparison winner",
      claimCandidate({
        safe_candidate_summary: "Take 2 is the winner and recommendation",
        claim_type: "comparison_public_result",
        claim_family: "comparison_public_result",
        blocked_claim_category: "public_comparison_result",
        blocker_codes: ["public_comparison_result_blocked"],
      }),
      "public_comparison_result",
  ...
  it("classifies missing evidence, unresolved truth and legacy-only anchor support as non-satisfying", async () => {
    const missingEvidence = await emitPublicClaimSupportBundle({
      candidates: [claimCandidate({ linked_evidence_anchor_ids: [] })],
    });
    expect(missingEvidence.claims.claims[0].support_status).toBe("missing_evidence");
    expect(missingEvidence.claims.claims[0].blocker_codes).toContain(
      "missing_evidence_anchor_support",
    );

    const missingTruth = await emitPublicClaimSupportBundle({
      truthStateMap: { truth_state_ids: [] },
    });
    ...
    expect(legacyAnchor.claims.claims[0].blocker_codes).toContain(
  ...
  it("supports factual brief presence and media readiness limitation claims when linked to real anchors and truth", async () => {
    const { claims } = await emitPublicClaimSupportBundle({
      ...
      anchors: [
        runtimeAnchor(),
        runtimeAnchor({
          evidence_anchor_id: "ea-media-limitation",
          safe_evidence_summary: "audio evidence family not extracted",
          evidence_text: "audio evidence family not extracted",
          linked_truth_state_ids: [],
          evidence_modality: "audio",
        }),
      ],
    });
    expect(claims.claims.map((claim: any) => claim.support_status)).toEqual([
      "supported",
  ...
  it("allows limitation claim support while broader EvidenceAnchors aggregate is partial, but keeps aggregate PublicClaimTrace insufficient when other claims are unsupported", async () => {
    const { claims } = await emitPublicClaimSupportBundle({
      ...
      candidates: [
        ...
        claimCandidate({
          claim_candidate_id: "cc-strength",
          safe_candidate_summary: "preserve the opening beat",
          claim_type: "preserve_strength",
          claim_family: "preserve_strength",
          linked_evidence_anchor_ids: ["ea-limitation"],
          linked_truth_state_ids: [],
      ...
      anchors: [
        runtimeAnchor({
          evidence_anchor_id: "ea-limitation",
          linked_truth_state_ids: [],
          safe_evidence_summary: "video evidence family not extracted",
        }),
      ],
    });
    ...
    expect(claims.claims[1].blocker_codes).toContain(
      "limitation_only_evidence_cannot_support_non_limitation_claim",
    );
    expect(claims.public_claim_gate_status).toBe("insufficient");
  });
    ...
    const { claims, metrics } = await emitPublicClaimSupportBundle({
      candidates: [
        claimCandidate({
          claim_type: "priority_fix",
          claim_family: "priority_fix",
          safe_candidate_summary: "adjust the eyeline",
          linked_evidence_anchor_ids: [],
        }),
      ],
    });
  ...
  it("redacts diagnostics and aligns manifest and qa_acceptance_metrics for insufficient and sufficient cases", async () => {
    const insufficient = await emitPublicClaimSupportBundle({
      candidates: [
        claimCandidate({
          safe_candidate_summary: "Use https://example.test/video?token=abc",
          linked_evidence_anchor_ids: [],
        }),
      ],
    });
    ...
    expect(serialized).toContain("[redacted unsafe candidate summary]");
    expect(serialized).not.toContain("token=abc");
    expect(insufficient.manifest.artefact_source_classification_by_id.public_claim_trace).toBe(
      insufficient.claims.source_classification,
    );
    expect(insufficient.metrics.public_claim_trace_summary.public_claim_gate_status).toBe(
      "insufficient",
    );

    const sufficient = await emitPublicClaimSupportBundle();
    expect(sufficient.manifest.artefact_source_classification_by_id.public_claim_trace).toBe(
      "real_runtime_v3_claim_support",
    );
    expect(sufficient.metrics.public_claim_gate_status).toBe("sufficient");
    expect(sufficient.metrics.public_claim_gate_reason).toBe(
  ...
  it("keeps global gates blocked and handles malformed ClaimCandidateTrace without crashing", async () => {
    const { claims, metrics } = await emitPublicClaimSupportBundle({
      candidates: [
        {
          malformed: true,
          linked_evidence_anchor_ids: ["missing-anchor"],
          source_family: "real_runtime_v3",
        },
      ],
      truthStateMap: null,
...
describe("S9-14M final runtime evidence promotion audit guardrail", () => {
  it("wires ClaimCandidateTrace into process-take PublicClaimTrace support classification", async () => {
    ...
    expect(source).not.toContain(
      "const publicClaimTraceClaims = publicClaimTrace.written && Array.isArray(publicClaimTrace.claims)",
    );
    expect(source).toContain("run_id: `take-${takeId}`");
    expect(source).toContain("take_id: takeId");
    expect(source).toMatch(
      /evidence_anchor_gate_status:\s*evidenceAnchors\.evidence_anchor_trace_summary\?\.evidence_anchor_gate_status/,
    );
    expect(source).toContain(
      "evidence_anchor_trace_summary: evidenceAnchors.evidence_anchor_trace_summary",
    );
    expect(source).toMatch(
      /evidence_anchor_source_family_summary:\s*evidenceAnchors\.evidence_anchor_trace_summary\?\.source_family_summary/,
    );
    expect(source).toContain(
      "evidence_anchor_trace_summary: evidenceAnchors.evidence_anchor_trace_summary,",
    );
    expect(source).toMatch(/evidence_family_coverage:\s*evidenceAnchors\.evidence_family_coverage/);
    expect(source).toMatch(
      /evidence_family_status_by_id:\s*evidenceAnchors\.evidence_family_status_by_id/,
    );
    expect(source).toMatch(
      /unsupported_or_unavailable_evidence:\s*evidenceAnchors\.unsupported_or_unavailable_evidence/,
    );
    expect(source).toMatch(/blocker_codes:\s*evidenceAnchors\.blocker_codes/);
    expect(source).toMatch(/cannot_satisfy_v3_gate:\s*evidenceAnchors\.cannot_satisfy_v3_gate/);
    ...
    expect(source).toMatch(
      /step1Dependency\.step1QaPersistenceStatus\s*===\s*["']failed_emission["']/,
    );
    ...
    expect(source).toMatch(
      /claim_candidate_trace_summary:\s*claimCandidateTraceDataForRuntimeTraces\s*\?\s*claimCandidateTrace\.summary\s*:\s*undefined/,
    );
  ...
  it("keeps valid in-memory runtime trace payloads available when QA sink writes fail", async () => {
    ...
    const candidateTraceData = {
      ...
      claim_candidate_source_summary: expectPresent(candidateOut.summary, "claim candidate summary")
    ...
    const publicClaimsOut = await emitPublicClaimTraceFirstPass({
      ...
      truth_state_map_data: {
        ...
        take_id: take,
        truth_state_ids: [
          ...new Set(anchors.flatMap((anchor: any) => anchor.linked_truth_state_ids ?? [])),
        ],
      },
      root_dir: root,
      internal_qa_emit: true,
    ...
    expect(
      "public_claim_gate_status" in publicClaimSummary
  ...
  it("bases resolver and TruthStateMap Step 2 availability on payload identity instead of emitted ids", async () => {
    ...
    expect(
      hasValidResolverOutputForStep2({ ...resolverOutput, run_id: "take-other" }, identity),
    ).toBe(false);
    expect(
      hasValidTruthStateMapForStep2({ ...truthStateMap, analysis_run_id: "take-other" }, identity),
    ).toBe(false);
    expect(
      hasValidResolverOutputForStep2(
        { artefact_type: "resolver_output", run_id: "take-t1" },
        identity,
      ),
    ).toBe(false);
  ...
  it.each([
    ...
    "rejects mismatched ClaimCandidateTrace %s before support classification",
    async (_field, overrides) => {
      const { claims } = await emitPublicClaimSupportBundle({
        claimCandidateTraceOverrides: overrides,
      });
      expect(claims.public_claim_gate_status).toBe("insufficient");
      expect(claims.cannot_satisfy_public_claim_gate).toBe(true);
  ...
  it("rejects candidate-level identity conflicts and missing ClaimCandidateTrace identity", async () => {
    ...
    expect(candidateConflict.claims.blocker_codes).toContain(
      "claim_candidate_trace_candidate_identity_mismatch",
    );
    ...
    expect(missingIdentity.claims.blocker_codes).toContain(
  ...
  it("lets PublicClaimTrace see sufficient EvidenceAnchors aggregate support in controlled fixtures", async () => {
    const { claims, metrics } = await emitPublicClaimSupportBundle();
    expect(claims.claims[0].support_status).toBe("supported");
    expect(claims.claims[0].evidence_support_summary.evidence_anchor_gate_status).toBe(
      "sufficient",
    );
    expect(claims.public_claim_gate_status).toBe("sufficient");
    expect(metrics.public_claim_gate_status).toBe("sufficient");
  ...
  it("keeps PublicClaimTrace insufficient when EvidenceAnchors aggregate data is partial or absent", async () => {
    const partial = await emitPublicClaimSupportBundle({
      evidenceAnchorGateStatus: "insufficient",
    });
    ...
    const missingMetadataRoot = await mkdtemp(
      path.join(os.tmpdir(), "qa-s914m-missing-anchor-metadata-"),
    );
  ...
  it("excludes unlinked internal observation candidates from aggregate public-claim requiredness", async () => {
    ...
    const observationClaim = claims.claims.find(
      (claim: any) => claim.claim_id === "pc-observation-only",
    );
    ...
    expect(claims.public_claim_gate_status).toBe("sufficient");
    expect(
      claims.claims.find((claim: any) => claim.claim_id === "pc-brief-presence").support_status,
    ).toBe("supported");
  });
  ...
  it("keeps raw-report ClaimCandidateTrace candidates diagnostic when v3 claim support is complete", async () => {
    ...
    await emitClaimCandidateTrace({
      ...
      raw_report_data: {
        report_data: {
          submission_verdict: {
            label: "Submit this take",
            reason: "Ready from legacy report wording",
          },
    ...
    const claimTracePath = path.join(
      bundle.root,
      bundle.run,
      "takes",
      `take-${bundle.take}`,
    ...
    const legacyCandidates = claimTrace.claim_candidates.filter(
      (candidate: any) => candidate.source_family === "legacy_adapter",
    );
    expect(legacyCandidates.length).toBeGreaterThan(0);
    expect(
      legacyCandidates.every(
        (candidate: any) => candidate.excluded_from_public_claim_gate === true,
      ),
    ).toBe(true);
    expect(
      legacyCandidates.every(
        (candidate: any) => candidate.required_for_public_claim_gate === false,
      ),
    ).toBe(true);
    expect(
      legacyCandidates.every(
        (candidate: any) => candidate.eligible_for_public_claim_trace_support_check === false,
      ),
    ).toBe(true);
    expect(
      legacyCandidates.every(
        (candidate: any) =>
          candidate.candidate_support_precheck_status === "legacy_diagnostic_only",
      ),
    ).toBe(true);
    ...
    const claimsOut = await emitPublicClaimTraceFirstPass({
      ...
      truth_state_map_data: {
        ...
        take_id: bundle.take,
        truth_state_ids: [
          ...new Set(anchors.anchors.flatMap((anchor: any) => anchor.linked_truth_state_ids ?? [])),
        ],
      },
      root_dir: bundle.root,
      internal_qa_emit: true,
    });
    const publicClaimTracePath = path.join(
      bundle.root,
      bundle.run,
      "takes",
      `take-${bundle.take}`,
    ...
    const legacyClaims = publicClaimTrace.claims.filter(
      (claim: any) => claim.source_family === "legacy_adapter",
    );
    ...
    expect(legacyClaims.every((claim: any) => claim.excluded_from_public_claim_gate === true)).toBe(
      true,
    );
    expect(legacyClaims.every((claim: any) => claim.required_for_public_claim_gate === false)).toBe(
      true,
    );
    expect(publicClaimTrace.blocker_codes).not.toContain(
  ...
  it("marks generated unlinked observable evidence candidates as internal-only and non-required", async () => {
    ...
    const observation = claimCandidateTrace.claim_candidates.find((candidate: any) =>
      String(candidate.source_path).startsWith("observable_evidence_items["),
    );
  ...
  it("does not resolve linked truth IDs from structural TruthStateMap keys", async () => {
    const structural = await emitPublicClaimSupportBundle({
      ...
      truthStateMap: {
        known_truths: { brief_presence: "supplied" },
        brief_truths: { status: "known" },
      },
    });
    expect(structural.claims.claims[0].support_status).toBe("missing_truth_link");
    expect(structural.claims.claims[0].truth_support_summary.unresolved_truth_state_ids).toEqual([
      "known_truths",
    ]);
  ...
  it("resolves explicit truth_state_id and truth_state_ids values only", async () => {
    const explicitId = await emitPublicClaimSupportBundle({
      ...
      truthStateMap: {
        known_truths: [{ truth_state_id: "take-x:truth_state:brief_001", truth_state: "known" }],
      },
    });
  ...
  it("resolves only canonical ID-keyed truth maps, not generic keyed maps", async () => {
    ...
    expect(generic.claims.claims[0].truth_support_summary.unresolved_truth_state_ids).toEqual([
      "brief_presence",
    ]);
  });
  ...
  it("records timestamp normalisation warnings without blocking AnalysisEvidenceState emission", async () => {
    ...
    expect(out.written).toBe(true);
    const analysisPayload = expectPresent(out.payload, "analysis evidence payload");
    expect(analysisPayload.timestamp_normalisation_warnings).toEqual([
      "take_created_at_invalid_timestamp",
    ]);
    expect(analysisPayload.assessability_limitations).toContain(
      "take_created_at_invalid_timestamp",
    );
  ...
  it("uses mixed limitation normalisation for known string-array limitation fields", async () => {
    const source = await readFile(
      path.join(process.cwd(), "src/server/v3/qa-artifacts-wiring.server.ts"),
      "utf8",
    );
    expect(source).toContain("normaliseSafeLimitationItems");
    expect(source).not.toContain(
      "safeRecordArray(analysisEvidenceState.assessability_limitations)",
    );
    expect(source).not.toContain(
      "safeRecordArray(analysisEvidenceState.timestamp_normalisation_warnings)",
    );
    expect(source).not.toContain(
      "safeRecordArray(analysisEvidenceState.timestamp_normalization_warnings)",
    );
  });
});
