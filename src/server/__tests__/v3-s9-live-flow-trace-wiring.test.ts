...
describe('v3 s9 live flow trace wiring', () => {
  it('A/D/F: combined flow emits traces and manifest/metrics agree while gate posture stays blocked', async () => {
    ...
    const scoreTrace = await emitScoreTraceFirstPass({ run_id: run, analysis_run_id: run, submission_id: 'sub1', take_id: take, source_module: 'test', source_stage: 'unit', raw_report_data: wrapped, public_claim_trace_data: { claims: claims.claims ?? [] }, root_dir: root, internal_qa_emit: true });
    await emitQAManifestForAnalysisRun({ run_id: run, analysis_run_id: run, take_id: take, submission_id: 'sub1', root_dir: root, internal_qa_emit: true, emitted_artefact_ids: ['raw_report', ...(anchors.written ? ['evidence_anchors'] : []), ...(claims.written ? ['public_claim_trace'] : []), ...(scoreTrace.written ? ['score_trace'] : [])], artefact_source_classification_by_id: { raw_report: 'legacy_adapter', evidence_anchors: 'legacy_adapter', public_claim_trace: 'legacy_adapter', ...(scoreTrace.written ? { score_trace: 'legacy_adapter' } : {}) }, artefact_level2_spine_satisfaction_by_id: { raw_report: false, evidence_anchors: false, public_claim_trace: false, ...(scoreTrace.written ? { score_trace: false } : {}) }, legacy_adapter_artefact_ids: ['raw_report', 'evidence_anchors', 'public_claim_trace', ...(scoreTrace.written ? ['score_trace'] : [])], public_claim_trace_summary: claims.summary });
