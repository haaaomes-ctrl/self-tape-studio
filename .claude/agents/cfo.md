...
tools: Read, Grep, Glob, Skill, mcp__supabase__execute_sql, mcp__supabase__list_tables
---
...
- **No database mutation.** SELECT and views only. Never INSERT/UPDATE/DELETE, never DDL,
  never `apply_migration`, never an edge-function deploy, never a branch/merge/reset.
...
1. Read `tc-finance-snapshot` (the skill carries the grounded query set and the snapshot
   ...
   `list_tables` / an `information_schema` SELECT before relying on it — never invent a
   table, column or view.
2. Run **SELECT-only** queries against the DS-16/DS-17 (and DS-04/06/13) views. Keep each
   figure paired with the query that produced it.
