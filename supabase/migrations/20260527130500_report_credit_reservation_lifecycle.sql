...
--
-- DS-04 created the funded-credit ledger foundation. This migration wires a
-- report-generation hold lifecycle around that ledger so one TapeCoach credit
-- is reserved before analysis, consumed only after a report is persisted, and
-- restored when report generation fails or is cancelled before analysis.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'takes_credit_reservation_id_fkey'
  ) THEN
    ALTER TABLE public.takes
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'takes_credit_lifecycle_status_known'
  ) THEN
    ALTER TABLE public.takes
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'takes_credit_lifecycle_metadata_object'
  ) THEN
    ALTER TABLE public.takes
...
BEGIN
  ...
  IF selected_take.credit_reservation_id IS NOT NULL THEN
    SELECT id INTO existing_reservation_id
    FROM public.report_credit_reservations
    WHERE id = selected_take.credit_reservation_id
      AND status = 'reserved'
    LIMIT 1;
  ...
  IF existing_reservation_id IS NOT NULL THEN
    ...
    SET
      ...
      credit_lifecycle_status = CASE
        WHEN p_synthetic_usage THEN 'synthetic_reserved'
        ELSE 'reserved'
      END,
  ...
  IF p_synthetic_usage THEN
    INSERT INTO public.report_credit_reservations (
      ...
      credit_amount,
      synthetic_usage,
      commercial_metrics_excluded,
      idempotency_key,
      metadata
    )
    VALUES (
      selected_take.user_id,
      selected_take.audition_id,
      selected_take.id,
      'reserved',
      1,
      TRUE,
    ...
    UPDATE public.takes
    SET
      credit_reservation_id = new_reservation_id,
  ...
  IF NOT FOUND THEN
    UPDATE public.takes
    SET
      credit_lifecycle_status = 'blocked_no_credit',
  ...
  SET
    ...
    status = CASE
      WHEN remaining_credits - 1 = 0 THEN 'exhausted'::public.credit_grant_status
      ELSE status
    END
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'report credit reservation not found';
  END IF;
  ...
  IF selected_reservation.synthetic_usage THEN
    ...
    SET
      status = 'consumed',
      consumed_at = p_report_generated_at,
    ...
    UPDATE public.takes
    SET
      credit_lifecycle_status = 'synthetic_consumed',
      credit_lifecycle_metadata = credit_lifecycle_metadata || effective_metadata || '{"synthetic_usage":true,"commercial_metrics_excluded":true}'::jsonb,
      credit_is_synthetic_usage = TRUE
  ...
  IF p_idempotency_key IS NOT NULL THEN
    ...
    IF existing_entry_id IS NOT NULL THEN
      new_entry_id := existing_entry_id;
    END IF;
  END IF;
  ...
  IF new_entry_id IS NULL THEN
    ...
    VALUES (
      selected_reservation.user_id,
      selected_reservation.source,
      'consume',
      -selected_reservation.credit_amount,
      selected_reservation.credit_grant_id,
      selected_reservation.take_id,
      selected_reservation.audition_id,
  ...
  UPDATE public.takes
  SET
    credit_consumption_ledger_entry_id = new_entry_id,
...
BEGIN
  ...
  IF selected_reservation.status IN ('released', 'refunded') THEN
  ...
  IF NOT selected_reservation.synthetic_usage AND selected_reservation.credit_grant_id IS NOT NULL THEN
    UPDATE public.credit_grants
    SET
      remaining_credits = LEAST(original_credits, remaining_credits + selected_reservation.credit_amount),
      status = CASE
        WHEN status = 'exhausted' THEN 'active'::public.credit_grant_status
        ELSE status
      END
  ...
  UPDATE public.takes
  SET
    credit_lifecycle_status = take_lifecycle_status,
    credit_lifecycle_metadata = credit_lifecycle_metadata || effective_metadata,
...
REVOKE EXECUTE ON FUNCTION public.reserve_report_credit_for_take(
  UUID,
  UUID,
  BOOLEAN,
  JSONB,
...
REVOKE EXECUTE ON FUNCTION public.consume_report_credit_reservation(
  UUID,
  UUID,
  TIMESTAMPTZ,
  JSONB,
...
REVOKE EXECUTE ON FUNCTION public.release_report_credit_reservation(
  UUID,
  public.report_credit_reservation_status,
  TEXT,
  TEXT,
  JSONB
) FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.reserve_report_credit_for_take(
  UUID,
  UUID,
  BOOLEAN,
  JSONB,
...
GRANT EXECUTE ON FUNCTION public.consume_report_credit_reservation(
  UUID,
  UUID,
  TIMESTAMPTZ,
  JSONB,
...
GRANT EXECUTE ON FUNCTION public.release_report_credit_reservation(
  UUID,
  public.report_credit_reservation_status,
  TEXT,
  TEXT,
