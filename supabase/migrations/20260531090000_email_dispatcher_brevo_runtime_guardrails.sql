...
--
-- This is a forward-only correction for environments where the CRM lifecycle
-- migration may already have been applied. Brevo owns transactional delivery;
-- the legacy /lovable/email/queue/process route path remains only for cron
-- compatibility.
...
SELECT
  'email_dispatcher_guardrails' AS check_name,
  dispatcher_mode,
  dispatcher_mode = 'disabled' AS safe_default
