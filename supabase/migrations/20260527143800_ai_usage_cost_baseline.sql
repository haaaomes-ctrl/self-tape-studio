-- DS-16: persisted AI usage instrumentation and cost baseline.
--
-- This records metadata about AI calls for unit economics. It intentionally
-- does not store raw prompts, raw model responses, media URLs, signed URLs or
-- performer-facing report prose.

DO $$
BEGIN
  CREATE TYPE public.ai_usage_step AS ENUM (
    'brief_extraction',
    'evidence_pass',
    'single_pass_report',
    'report_polish',
...
DO $$
BEGIN
  CREATE TYPE public.ai_usage_status AS ENUM (
    'success',
    'failure',
    'timeout',
    'cancelled'
...
DO $$
BEGIN
  CREATE TYPE public.ai_usage_cost_source AS ENUM (
    'planning_baseline',
    'duration_baseline',
...
CREATE OR REPLACE FUNCTION public.estimate_ai_report_cost_usd(
...
CREATE TABLE IF NOT EXISTS public.take_ai_usage (
  ...
  CONSTRAINT take_ai_usage_http_status_valid CHECK (
    http_status IS NULL OR (http_status >= 100 AND http_status <= 599)
  ),
  CONSTRAINT take_ai_usage_tokens_non_negative CHECK (
  ...
  CONSTRAINT take_ai_usage_duration_non_negative CHECK (
  ...
  CONSTRAINT take_ai_usage_status_success_consistent CHECK (
    (status = 'success' AND success = TRUE)
    OR (status <> 'success' AND success = FALSE)
  )
);
...
CREATE INDEX IF NOT EXISTS take_ai_usage_take_created_idx
  ON public.take_ai_usage (take_id, created_at DESC)
  WHERE take_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS take_ai_usage_audition_created_idx
  ON public.take_ai_usage (audition_id, created_at DESC)
  WHERE audition_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS take_ai_usage_step_status_idx
  ON public.take_ai_usage (step, status, created_at DESC);
CREATE INDEX IF NOT EXISTS take_ai_usage_model_created_idx
  ON public.take_ai_usage (provider, model, created_at DESC);
CREATE INDEX IF NOT EXISTS take_ai_usage_duration_status_idx
  ON public.take_ai_usage (duration_status, created_at DESC)
  WHERE duration_status IS NOT NULL;
...
latest_reservation AS (
  SELECT DISTINCT ON (reservation.take_id)
    ...
    reservation.source AS credit_source,
...
take_duration AS (
  SELECT
    ...
    COALESCE(
      take_row.mux_duration_seconds::NUMERIC,
      CASE
        WHEN (take_row.signals ->> 'duration_seconds') ~ '^[0-9]+(\.[0-9]+)?$'
          THEN (take_row.signals ->> 'duration_seconds')::NUMERIC
        WHEN (take_row.signals ->> 'duration') ~ '^[0-9]+(\.[0-9]+)?$'
          THEN (take_row.signals ->> 'duration')::NUMERIC
        WHEN (take_row.checklist #>> '{duration,seconds}') ~ '^[0-9]+(\.[0-9]+)?$'
          THEN (take_row.checklist #>> '{duration,seconds}')::NUMERIC
        ELSE NULL
      END
    ) AS duration_seconds
...
SELECT
  take_row.id AS take_id,
  take_row.audition_id,
  take_row.user_id,
  take_row.status AS take_status,
  take_row.created_at AS take_created_at,
  take_row.updated_at AS take_updated_at,
  ...
  COALESCE(
    usage_rollup.usage_duration_status,
    NULLIF(take_row.signals ->> 'duration_status', ''),
    CASE
  ...
  partner_context.partner_type,
  CASE
    WHEN COALESCE(usage_rollup.ai_call_count, 0) > 0 THEN usage_rollup.recorded_estimated_cost_usd
    ELSE public.estimate_ai_report_cost_usd(take_duration.duration_seconds)
  END AS report_estimated_cost_usd,
  CASE
    WHEN COALESCE(usage_rollup.ai_call_count, 0) > 0 THEN 'recorded_ai_usage'
    ELSE 'duration_baseline'
  END AS report_cost_source,
...
SELECT
  ...
  ROUND(COALESCE(percentile_cont(0.5) WITHIN GROUP (ORDER BY report_estimated_cost_usd), 0)::NUMERIC, 6)
    AS p50_report_cost_usd,
  ROUND(COALESCE(percentile_cont(0.95) WITHIN GROUP (ORDER BY report_estimated_cost_usd), 0)::NUMERIC, 6)
    AS p95_report_cost_usd,
  ...
  ROUND(
    COALESCE(SUM(fallback_call_count)::NUMERIC / NULLIF(SUM(ai_call_count), 0), 0),
    4
  ) AS fallback_rate,
  ROUND(
    COALESCE(SUM(repair_call_count)::NUMERIC / NULLIF(SUM(ai_call_count), 0), 0),
    4
  ) AS repair_rate,
...
SELECT
  COALESCE(credit_source::TEXT, CASE WHEN synthetic_usage THEN 'synthetic_usage' ELSE 'unknown' END)
    AS credit_source_group,
  partner_id,
  partner_name,
  partner_type,
  duration_status,
...
SELECT
  ...
  ROUND(COALESCE(percentile_cont(0.5) WITHIN GROUP (ORDER BY latency_ms), 0)::NUMERIC, 2)
    AS p50_latency_ms,
  ROUND(COALESCE(percentile_cont(0.95) WITHIN GROUP (ORDER BY latency_ms), 0)::NUMERIC, 2)
    AS p95_latency_ms,
