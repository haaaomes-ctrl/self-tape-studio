-- DS-10: Agent trial package presets.
--
-- This extends the shared package catalogue to limited agent trials only.
-- Public landing pages, report-share UI, full dashboards/admin UI and runtime
-- credit reservation are owned by later DS slices.
...
ALTER TABLE public.partner_package_presets
  ADD CONSTRAINT partner_package_presets_supported_context CHECK (
    ...
    (
      partner_type = 'coach'
      AND display_context = 'coach_pilot'
      AND billing_period = 'monthly'
      AND package_tier IN ('starter', 'studio')
    )
    OR
    (
      partner_type = 'agent'
      AND display_context = 'agent_trial'
      AND billing_period = 'monthly'
      AND package_tier IN ('trial', 'growth')
    )
  );
...
ALTER TABLE public.partner_package_presets
  ADD CONSTRAINT partner_package_presets_package_credits CHECK (
    ...
    (
      partner_type = 'coach'
      AND credits_per_member = per_user_cap
...
ALTER TABLE public.partner_package_presets
  ADD CONSTRAINT partner_package_presets_pool_visibility CHECK (
    (
      partner_type = 'school'
      AND pool_period_type = 'term'
      AND progress_visibility_scope = 'named_progress'
    )
    OR
    (
      partner_type = 'coach'
      AND pool_period_type = 'monthly'
      AND progress_visibility_scope = 'named_progress'
    )
    OR
    (
