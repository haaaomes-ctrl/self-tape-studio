-- DS-04: funded credit ledger, source attribution and finance reporting foundation.
--
-- This creates the ledger-first accounting model that DS-12 can later wire into
-- runtime reservation/refund enforcement. It intentionally does not gate report
-- generation, reserve credits before analysis, or add failed-report user copy.
...
DO $$
BEGIN
  CREATE TYPE public.credit_rollover_policy AS ENUM (
    'rollover',
    'no_rollover',
    'funding_period'
  );
...
CREATE TABLE IF NOT EXISTS public.credit_grants (
  ...
  CONSTRAINT credit_grants_remaining_valid CHECK (
    remaining_credits >= 0 AND remaining_credits <= original_credits
  ),
  CONSTRAINT credit_grants_free_monthly_expiry CHECK (
    source <> 'free_monthly' OR (rollover_policy = 'no_rollover' AND expires_at IS NOT NULL)
  ),
  CONSTRAINT credit_grants_user_paid_rollover CHECK (
    source <> 'user_paid' OR (rollover_policy = 'rollover' AND expires_at IS NULL)
  ),
...
CREATE TABLE IF NOT EXISTS public.credit_ledger_entries (
  ...
  CONSTRAINT credit_ledger_grant_positive CHECK (
    entry_type <> 'grant' OR (credit_delta > 0 AND credit_grant_id IS NOT NULL)
  ),
  CONSTRAINT credit_ledger_consume_negative CHECK (
    entry_type <> 'consume' OR (credit_delta < 0 AND credit_grant_id IS NOT NULL)
  ),
  CONSTRAINT credit_ledger_expiry_negative CHECK (
    entry_type <> 'expiry_adjustment' OR credit_delta < 0
  ),
  CONSTRAINT credit_ledger_consume_report_link CHECK (
    entry_type <> 'consume' OR take_id IS NOT NULL OR source_reference_type IS NOT NULL
  ),
  CONSTRAINT credit_ledger_metadata_object CHECK (jsonb_typeof(metadata) = 'object')
);

ALTER TABLE public.takes
  ADD COLUMN IF NOT EXISTS credit_consumption_ledger_entry_id UUID;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'takes_credit_consumption_ledger_entry_id_fkey'
  ) THEN
    ALTER TABLE public.takes
...
CREATE TRIGGER credit_ledger_entries_set_updated_at
BEFORE UPDATE ON public.credit_ledger_entries
FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

CREATE INDEX IF NOT EXISTS credit_grants_user_status_expiry_idx
  ON public.credit_grants (user_id, status, expires_at, granted_at);
CREATE INDEX IF NOT EXISTS credit_grants_source_created_idx
  ON public.credit_grants (source, created_at DESC);
CREATE INDEX IF NOT EXISTS credit_ledger_entries_user_created_idx
  ON public.credit_ledger_entries (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS credit_ledger_entries_source_created_idx
  ON public.credit_ledger_entries (source, created_at DESC);
CREATE INDEX IF NOT EXISTS credit_ledger_entries_grant_idx
  ON public.credit_ledger_entries (credit_grant_id);
CREATE INDEX IF NOT EXISTS credit_ledger_entries_take_idx
  ON public.credit_ledger_entries (take_id);
CREATE INDEX IF NOT EXISTS takes_credit_consumption_ledger_entry_idx
  ON public.takes (credit_consumption_ledger_entry_id)
  WHERE credit_consumption_ledger_entry_id IS NOT NULL;
...
SELECT
  source,
  COALESCE(SUM(CASE WHEN entry_type = 'grant' THEN credit_delta ELSE 0 END), 0)::INTEGER
    AS granted_credits,
  ABS(COALESCE(SUM(CASE WHEN entry_type = 'consume' THEN credit_delta ELSE 0 END), 0))::INTEGER
    AS consumed_credits,
  COALESCE(SUM(CASE WHEN entry_type = 'admin_adjustment' THEN credit_delta ELSE 0 END), 0)::INTEGER
    AS admin_adjustment_credits,
  ABS(COALESCE(SUM(CASE WHEN entry_type = 'expiry_adjustment' THEN credit_delta ELSE 0 END), 0))::INTEGER
    AS expired_credits,
...
BEGIN
  ...
  IF p_idempotency_key IS NOT NULL THEN
    SELECT credit_grant_id INTO existing_grant_id
    FROM public.credit_ledger_entries
  ...
  ELSIF p_source = 'user_paid' THEN
    ...
    effective_rollover_policy := 'rollover';
  ELSIF p_expires_at IS NOT NULL THEN
    effective_expires_at := p_expires_at;
    effective_rollover_policy := 'funding_period';
  ELSE
    effective_expires_at := NULL;
    effective_rollover_policy := 'rollover';
  END IF;
...
BEGIN
  IF p_credit_amount <= 0 THEN
    RAISE EXCEPTION 'credit consumption amount must be positive';
  END IF;
  ...
  IF selected_grant.user_id <> p_user_id THEN
    RAISE EXCEPTION 'credit grant does not belong to user';
  END IF;

  IF selected_grant.status <> 'active' THEN
    RAISE EXCEPTION 'credit grant is not active';
  END IF;

  IF selected_grant.expires_at IS NOT NULL AND selected_grant.expires_at <= now() THEN
    RAISE EXCEPTION 'credit grant is expired';
  END IF;

  IF selected_grant.remaining_credits < p_credit_amount THEN
    RAISE EXCEPTION 'insufficient remaining credits on grant';
  END IF;
  ...
  SET
    remaining_credits = remaining_credits - p_credit_amount,
    status = CASE
      WHEN remaining_credits - p_credit_amount = 0 THEN 'exhausted'::public.credit_grant_status
      ELSE status
    END
...
BEGIN
  IF p_credit_delta = 0 THEN
    RAISE EXCEPTION 'admin credit adjustment delta must not be zero';
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.grant_funded_credits(
  UUID,
  public.credit_source,
  INTEGER,
  TIMESTAMPTZ,
  TIMESTAMPTZ,
  TEXT,
...
REVOKE EXECUTE ON FUNCTION public.record_credit_consumption(
  UUID,
  UUID,
  INTEGER,
  UUID,
...
REVOKE EXECUTE ON FUNCTION public.record_admin_credit_adjustment(
  UUID,
  public.credit_source,
  INTEGER,
  UUID,
...
GRANT EXECUTE ON FUNCTION public.grant_funded_credits(
  UUID,
  public.credit_source,
  INTEGER,
  TIMESTAMPTZ,
...
GRANT EXECUTE ON FUNCTION public.record_credit_consumption(
  UUID,
  UUID,
  INTEGER,
  UUID,
...
GRANT EXECUTE ON FUNCTION public.record_admin_credit_adjustment(
  UUID,
  public.credit_source,
  INTEGER,
  UUID,
