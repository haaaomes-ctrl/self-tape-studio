...
BEGIN
  ...
  IF user_email IS NULL OR trim(user_email) = '' THEN RETURN NULL; END IF;
  SELECT * INTO compliance FROM public.account_compliance WHERE user_id = p_user_id;
  IF NOT FOUND THEN RETURN NULL; END IF;
  normalized := public.crm_normalize_email(user_email);
  IF compliance.account_type = 'parent_guardian_managed' THEN
    segment := 'parent_guardian'; role := 'parent_guardian';
  ELSIF compliance.account_type = 'self_service_performer' THEN
    segment := 'performer'; role := 'performer';
  END IF;
  INSERT INTO public.crm_contacts (
    user_id, email, normalized_email, user_segment, recipient_role,
    account_route, account_type, age_band_declaration, parent_managed,
    service_messages_allowed, marketing_consent, marketing_consent_at,
    lifecycle_messages_allowed, consent_source, crm_metadata
  ) VALUES (
    p_user_id, user_email, normalized, segment, role,
    compliance.account_route, compliance.account_type, compliance.age_band_declaration,
    COALESCE(compliance.parent_managed, false), true,
    COALESCE(compliance.marketing_consent, false), compliance.marketing_consent_at,
    COALESCE(compliance.marketing_consent, false), 'account_compliance',
    jsonb_build_object(
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    user_segment = EXCLUDED.user_segment, recipient_role = EXCLUDED.recipient_role,
    account_route = EXCLUDED.account_route, account_type = EXCLUDED.account_type,
...
BEGIN
  ...
  SELECT token INTO existing_token FROM public.email_unsubscribe_tokens WHERE email = normalized;
  IF existing_token IS NOT NULL THEN RETURN existing_token; END IF;
  next_token := replace(gen_random_uuid()::TEXT, '-', '');
  INSERT INTO public.email_unsubscribe_tokens (token, email) VALUES (next_token, normalized)
...
CREATE OR REPLACE FUNCTION public.crm_build_unsubscribe_url(
  p_token TEXT, p_base_url TEXT DEFAULT 'https://tapecoach.co.uk/unsubscribe'
) RETURNS TEXT LANGUAGE SQL IMMUTABLE AS $$
  SELECT
    CASE
...
CREATE OR REPLACE FUNCTION public.enqueue_crm_lifecycle_email(
  p_user_id UUID, p_message_key TEXT, p_message_category TEXT, p_subject TEXT,
  p_preview_text TEXT DEFAULT '', p_html TEXT DEFAULT NULL, p_text TEXT DEFAULT NULL,
  p_template_data JSONB DEFAULT '{}'::jsonb, p_idempotency_key TEXT DEFAULT NULL,
  p_send_after TIMESTAMPTZ DEFAULT now(), p_from TEXT DEFAULT NULL,
  p_sender_domain TEXT DEFAULT 'notify.tapecoach.co.uk',
  p_purpose TEXT DEFAULT 'transactional', p_label TEXT DEFAULT NULL,
...
BEGIN
  IF p_message_key NOT IN (
    ...
    'no_report_started','report_started','report_ready','failed_report_credit_restored',
    'credits_added','b2b_follow_up'
  ) THEN RAISE EXCEPTION 'unsupported CRM message key'; END IF;
  IF p_message_category NOT IN ('service','lifecycle','marketing') THEN
    RAISE EXCEPTION 'unsupported CRM message category';
  END IF;
  IF p_idempotency_key IS NOT NULL THEN
    SELECT log.message_id INTO message_id FROM public.email_send_log log
  ...
  IF contact.user_id IS NULL THEN RAISE EXCEPTION 'CRM contact is unavailable for user'; END IF;
  ...
  ELSIF public.crm_message_category_requires_consent(p_message_category) AND is_suppressed THEN
  ...
  IF suppression_reason IS NOT NULL THEN
    ...
    VALUES (
      message_id, COALESCE(NULLIF(trim(p_label), ''), p_message_key), contact.email,
      'suppressed', suppression_reason,
      jsonb_build_object('message_key', p_message_key, 'message_category', p_message_category,
  ...
  IF unsubscribe_url IS NOT NULL THEN
    base_html := base_html || '<hr><p style="font-size:12px;color:#6b7280;">You can unsubscribe from TapeCoach lifecycle emails at any time: <a href="' || unsubscribe_url || '">unsubscribe</a>.</p>';
    base_text := base_text || E'\n\nUnsubscribe from TapeCoach lifecycle emails: ' || unsubscribe_url;
  END IF;
  payload := jsonb_build_object(
    'run_id', message_id, 'message_id', message_id, 'to', contact.email,
    ...
    'subject', trim(p_subject), 'html', base_html, 'text', base_text,
    'purpose', COALESCE(NULLIF(trim(p_purpose), ''), 'transactional'),
    'label', COALESCE(NULLIF(trim(p_label), ''), p_message_key),
    'idempotency_key', message_id, 'unsubscribe_token', unsubscribe_token,
    'queued_at', COALESCE(p_send_after, now()), 'template_data', safe_template_data
  );
  ...
  VALUES (
    message_id, COALESCE(NULLIF(trim(p_label), ''), p_message_key), contact.email, 'pending',
    jsonb_build_object('message_key', p_message_key, 'message_category', p_message_category,
      'source','crm_lifecycle','user_segment', contact.user_segment,
      'recipient_role', contact.recipient_role)
  );
...
SELECT
  contact.user_segment, contact.recipient_role, contact.account_route,
  contact.parent_managed, contact.marketing_consent, contact.lifecycle_messages_allowed,
...
GROUP BY contact.user_segment, contact.recipient_role, contact.account_route,
  contact.parent_managed, contact.marketing_consent, contact.lifecycle_messages_allowed,
  contact.brevo_sync_status;
...
WITH latest_log AS (
  SELECT DISTINCT ON (COALESCE(log.message_id, log.id::TEXT))
    ...
    log.template_name, log.recipient_email, log.status, log.error_message,
...
GROUP BY date_trunc('day', latest_log.created_at)::DATE,
  latest_log.template_name, COALESCE(latest_log.metadata ->> 'message_category', 'unknown'),
  latest_log.status;
...
WITH latest_log AS (
  SELECT DISTINCT ON (COALESCE(log.message_id, log.id::TEXT))
    COALESCE(log.message_id, log.id::TEXT) AS effective_message_id,
    log.template_name, log.status, log.metadata, log.created_at
  FROM public.email_send_log log
  ORDER BY COALESCE(log.message_id, log.id::TEXT), log.created_at DESC, log.id DESC
)
SELECT
...
GROUP BY COALESCE(latest_log.metadata ->> 'message_category', 'unknown'),
  latest_log.template_name;

CREATE OR REPLACE VIEW public.crm_b2b_leads_dashboard AS
SELECT
  leads.lead_day, leads.lead_type, leads.lead_type AS partner_type,
  leads.attribution_source AS utm_source, leads.utm_campaign,
  leads.creator_code, leads.partner_code_hint, leads.lead_count,
