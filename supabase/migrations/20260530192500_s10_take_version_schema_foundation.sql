-- S10-04: take slots, versions and comparison schema foundation.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'takes_s10_take_slot_check'
  ) THEN
    ALTER TABLE public.takes
      ADD CONSTRAINT takes_s10_take_slot_check
      CHECK (take_slot IS NULL OR take_slot BETWEEN 1 AND 3);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'takes_s10_take_version_number_check'
  ) THEN
    ALTER TABLE public.takes
      ADD CONSTRAINT takes_s10_take_version_number_check
      CHECK (take_version_number >= 1);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'takes_s10_take_version_status_check'
  ) THEN
    ALTER TABLE public.takes
      ADD CONSTRAINT takes_s10_take_version_status_check
      CHECK (
        take_version_status IN (
          'active',
          'replaced',
          'processing_failed',
          'analysis_failed',
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'takes_s10_replacement_reason_check'
  ) THEN
    ALTER TABLE public.takes
      ADD CONSTRAINT takes_s10_replacement_reason_check
      CHECK (
        replacement_reason IS NULL
        OR replacement_reason IN (
          'user_replaced',
          'operator_retest',
          'upload_cancelled',
          'same_media_retest',
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'takes_s10_report_model_status_check'
  ) THEN
    ALTER TABLE public.takes
      ADD CONSTRAINT takes_s10_report_model_status_check
      CHECK (report_model_status IN ('not_emitted', 'pending', 'rendered', 'failed', 'deferred'));
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'takes_s10_qa_artifact_status_check'
  ) THEN
    ALTER TABLE public.takes
      ADD CONSTRAINT takes_s10_qa_artifact_status_check
      CHECK (
        qa_artifact_status IN (
          'not_enabled',
          'emitted',
          'partially_emitted',
          'failed',
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'takes_s10_manifest_status_check'
  ) THEN
    ALTER TABLE public.takes
      ADD CONSTRAINT takes_s10_manifest_status_check
      CHECK (
        manifest_status IN (
          'not_enabled',
          'emitted',
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'takes_s10_same_video_status_check'
  ) THEN
    ALTER TABLE public.takes
      ADD CONSTRAINT takes_s10_same_video_status_check
      CHECK (
        same_video_status IN (
          'not_evaluated',
          'new_media',
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'takes_s10_lifecycle_metadata_object_check'
  ) THEN
    ALTER TABLE public.takes
      ADD CONSTRAINT takes_s10_lifecycle_metadata_object_check
...
CREATE INDEX IF NOT EXISTS takes_analysis_run_id_idx
  ON public.takes (analysis_run_id)
...
CREATE TABLE IF NOT EXISTS public.audition_comparison_runs (
  ...
  CONSTRAINT audition_comparison_runs_take_count_check
    CHECK (cardinality(compared_take_version_ids) <= 3),
  CONSTRAINT audition_comparison_runs_slot_count_check
    CHECK (cardinality(compared_take_slots) = cardinality(compared_take_version_ids)),
  CONSTRAINT audition_comparison_runs_slot_values_check
    CHECK (compared_take_slots <@ ARRAY[1, 2, 3]::integer[]),
  CONSTRAINT audition_comparison_runs_status_check
    CHECK (
  ...
  CONSTRAINT audition_comparison_runs_qa_artifact_status_check
    CHECK (
      qa_artifact_status IN (
        'not_enabled',
        'emitted',
  ...
  CONSTRAINT audition_comparison_runs_same_video_status_check
    CHECK (
      same_video_status IN (
        'not_evaluated',
        'new_media',
  ...
  CONSTRAINT audition_comparison_runs_qa_artifacts_object_check
    CHECK (jsonb_typeof(qa_artifacts) = 'object'),
  CONSTRAINT audition_comparison_runs_metadata_object_check
    CHECK (jsonb_typeof(comparison_metadata) = 'object')
);

CREATE INDEX IF NOT EXISTS audition_comparison_runs_audition_id_idx
  ON public.audition_comparison_runs (audition_id, created_at DESC);

CREATE INDEX IF NOT EXISTS audition_comparison_runs_user_id_idx
  ON public.audition_comparison_runs (user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS audition_comparison_runs_stale_idx
  ON public.audition_comparison_runs (audition_id, stale_after_replacement)
  WHERE stale_after_replacement = true;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'audition_comparison_runs'
      AND policyname = 'Owner can select audition comparison runs'
  ) THEN
    CREATE POLICY "Owner can select audition comparison runs"
      ON public.audition_comparison_runs FOR SELECT
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'audition_comparison_runs'
      AND policyname = 'Owner can insert audition comparison runs'
  ) THEN
    CREATE POLICY "Owner can insert audition comparison runs"
      ON public.audition_comparison_runs FOR INSERT
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'audition_comparison_runs'
      AND policyname = 'Owner can update audition comparison runs'
  ) THEN
    CREATE POLICY "Owner can update audition comparison runs"
      ON public.audition_comparison_runs FOR UPDATE
      USING (auth.uid() = user_id);
  END IF;
END $$;

DROP TRIGGER IF EXISTS audition_comparison_runs_set_updated_at
  ON public.audition_comparison_runs;
...
BEGIN
  ...
  IF old_take.user_id IS DISTINCT FROM p_user_id THEN
    RAISE EXCEPTION 'take_forbidden' USING ERRCODE = '42501';
  END IF;

  IF old_take.take_version_status IS DISTINCT FROM 'active' THEN
    RAISE EXCEPTION 'take_version_not_active' USING ERRCODE = '22023';
  END IF;

  resolved_slot := COALESCE(
    old_take.take_slot,
    CASE WHEN old_take.take_number BETWEEN 1 AND 3 THEN old_take.take_number END
  );

  IF resolved_slot IS NULL OR resolved_slot NOT BETWEEN 1 AND 3 THEN
    RAISE EXCEPTION 'invalid_take_slot' USING ERRCODE = '22023';
  END IF;

  IF p_replacement_reason NOT IN (
    'user_replaced',
    'operator_retest',
    'upload_cancelled',
    'same_media_retest',
  ...
  SELECT COALESCE(MAX(take_version_number), 0) + 1
  INTO next_version
  FROM public.takes
  WHERE audition_id = old_take.audition_id
    AND COALESCE(
      take_slot,
      CASE WHEN take_number BETWEEN 1 AND 3 THEN take_number END
    ) = resolved_slot;

  UPDATE public.takes
  SET
    take_slot = resolved_slot,
    take_version_status = 'replaced',
    replacement_reason = p_replacement_reason,
    report_model_status = CASE
      WHEN report IS NOT NULL THEN 'rendered'
      ELSE report_model_status
    END,
  ...
  VALUES (
    ...
    false,
    COALESCE(old_take.analytics_attribution, '{}'::jsonb),
    old_take.anon_id,
    'not_emitted',
    'not_enabled',
    'not_enabled',
    'not_evaluated',
    jsonb_build_object(
      'created_from_take_id', p_take_id,
      'replacement_reason', p_replacement_reason
    )
  )
  ...
  UPDATE public.takes
  SET replaced_by_take_id = replacement_take_id
  WHERE id = p_take_id;
