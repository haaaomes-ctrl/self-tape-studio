...
**Read-boundary (CFO).** The CFO adds one new access path: a **read-only** path to the owned Supabase finance data — the DS-16 AI-cost baseline and DS-17 CFO dashboard views (plus DS-04/06/13), **SELECT/views only**. It sits on top of that finance data and is **advisory**: it writes only finance snapshots to the corpus (`knowledge/60-finance/`, via `tc-finance-snapshot`) and never mutates money, pricing, Stripe or config. Finance recommendations go to the SRO and become BA items built by the engineer pair via PR. The CFO's DB path is now a **Postgres-protocol MCP** (`supabase-cfo-ro`, tool `mcp__supabase-cfo-ro__query`) that connects **as the `cfo_readonly` role** via a read-only DSN (env var `CFO_READONLY_DATABASE_URL`, set by the operator out-of-band). Both guarantees are now **structural at the connection**: **no-write** (the role has zero write grants, so DML/DDL is rejected as `permission denied`, and the MCP also runs each query read-only) and **finance-scoping** (as `cfo_readonly` it can `SELECT` only the 20 finance relations — `USAGE` + `SELECT` on those relations only, zero write, zero non-finance read; migration `20260609115531_cfo_readonly_finance_role.sql`, verified live). This closes the prior interim/contractual finance-scoping caveat (`60-finance/finance-area-readme.md`).
...
- **Tools/skills:** `Read`, `Grep`, `Glob`, `Skill`, `tc-finance-snapshot`, and a **read-only** Supabase SELECT path via the dedicated Postgres-protocol MCP (`mcp__supabase-cfo-ro__query`), which connects **as the `cfo_readonly` role** through a read-only DSN — both no-write and finance-scoping are structural at the connection (SELECT/views only). No mutation tools, and not the writable shared `supabase` MCP, by grant.
