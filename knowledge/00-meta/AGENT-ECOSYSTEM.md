...
**Read-boundary (CFO).** The CFO adds one new access path: a **read-only** path to the owned Supabase finance data — the DS-16 AI-cost baseline and DS-17 CFO dashboard views (plus DS-04/06/13), **SELECT/views only**. It sits on top of that finance data and is **advisory**: it writes only finance snapshots to the corpus (`knowledge/60-finance/`, via `tc-finance-snapshot`) and never mutates money, pricing, Stripe or config. Finance recommendations go to the SRO and become BA items built by the engineer pair via PR. The CFO's DB path is now the dedicated `read_only=true` Supabase MCP (`supabase-cfo-ro`), so **no-write is structural** (every query runs as a read-only Postgres user — not merely a contract), and a finance-scoped `cfo_readonly` role exists and is verified (`USAGE` + `SELECT` on the 20 finance relations only, zero write, zero non-finance read). The remaining follow-up is connecting **as** `cfo_readonly` via a direct DSN so finance-scoping is structural at the connection too — until then finance-scoping is a contract (the grounded query set) while no-write is structural (`60-finance/finance-area-readme.md`).
...
- **Tools/skills:** `Read`, `Grep`, `Glob`, `Skill`, `tc-finance-snapshot`, and a **read-only** Supabase SELECT path via the dedicated `read_only=true` MCP (`mcp__supabase-cfo-ro__execute_sql` / `mcp__supabase-cfo-ro__list_tables`, views only — no-write is structural). No mutation tools, and not the writable shared `supabase` MCP, by grant.
